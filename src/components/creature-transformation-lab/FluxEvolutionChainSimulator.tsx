...
export function FluxEvolutionChainSimulator({ creatureId }: { creatureId: string }) {
    ...
    const [minimalPrompt, setMinimalPrompt] = useState(true)
    ...
    const launch = useCallback(async (current: Chain, index: number) => {
        ...
        try {
            const response = await generateFluxEvolutionChainStep({
                ...
                promptTemplateVersion: current.promptTemplateVersion ?? 'flux-minimal-v1',
    ...
    return <section className="flux-chain-simulator" aria-labelledby="flux-chain-simulator-title">
        ...
        {chain ? <><div className="flux-chain-simulator__actions"><p role="status">Stato: {chain.status} · {chain.promptTemplateVersion ?? 'flux-minimal-v1'}{activeStep ? ` · G${activeStep.generation} ${activeStep.state}` : ''}</p>{chain.status === 'generating' ? <button type="button" onClick={() => { stoppedRef.current = true; setChain({ ...chain, status: 'stopped', steps: chain.steps.map((step) => step.state === 'pending' ? { ...step, state: 'stopped' } : step) }) }}>Stop simulation</button> : null}{chain.status === 'failed' ? <button type="button" onClick={retry}>Riprova step fallito</button> : null}</div><ol className="flux-chain-simulator__timeline"><li><span>BASE</span></li>{timeline.map((step) => <li key={step.generation} data-state={step.state}><button type="button" disabled={!step.imageUrl} onClick={() => step.imageUrl && setLargeImage({ url: step.imageUrl, label: `Generazione ${step.generation}` })}>{step.imageUrl ? <img src={step.imageUrl} alt={`Generazione ${step.generation}`} /> : <span aria-hidden="true">G{step.generation}</span>}<strong>G{step.generation}</strong></button><small>{targetLabel(step.targetId)} · {step.state}</small>{step.conceptName ? <em>{step.conceptName}</em> : null}{step.evolutionFunction ? <em>{step.evolutionFunction}</em> : null}{step.capability ? <em>{step.capability}</em> : null}{step.error ? <p role="alert">{step.error}</p> : null}</li>)}</ol></> : null}
