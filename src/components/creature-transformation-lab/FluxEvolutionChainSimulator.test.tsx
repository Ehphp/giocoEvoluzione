...
describe('FluxEvolutionChainSimulator', () => {
    ...
    it('can explicitly opt into flux-micro-v6 for full-prompt comparisons', async () => {
        const root = createRoot(container)
        await act(async () => { root.render(createElement(FluxEvolutionChainSimulator, { creatureId: 'creature-1' })) })
        ...
        expect(container.textContent).toContain('flux-micro-v6')
        expect(JSON.parse(window.localStorage.getItem('flux-evolution-chain-simulator:creature-1')!).promptTemplateVersion).toBe('flux-micro-v6')
        await act(async () => root.unmount())
    })
