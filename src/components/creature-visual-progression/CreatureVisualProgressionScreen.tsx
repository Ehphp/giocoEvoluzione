...
export function CreatureVisualProgressionScreen({ creature, onBack, onVisualChanged }: Props) {
    ...
    const refresh = useCallback(async () => {
        ...
        if (result.track?.status === 'GENERATED' && requestId) {
            ...
            if (status.result && status.productPreview) {
                ...
                setPreview(null)
                setError(status.error?.message ?? 'La proposta generata non e recuperabile. Non e possibile adottarla finché non viene validata dal server.')
            }
        } else {
            setPreview(null)
        }
    ...
    return (
        <section className="visual-progression-screen" aria-labelledby="visual-progression-title">
            ...
            {progress?.track?.status === 'GENERATED' && !preview ? <section className="visual-progression-screen__card"><h2>Anteprima in verifica</h2><p>La proposta esiste ma non è ancora recuperabile come asset ufficiale. Non è adottabile.</p><button type="button" disabled={busy} onClick={() => void refresh()}>Ricarica proposta</button></section> : null}
            ...
            {progress?.track && !['ACTIVE', 'READY', 'GENERATING', 'POST_PROCESSING', 'GENERATED', 'COMPLETED'].includes(progress.track.status) ? <section className="visual-progression-screen__card"><h2>Percorso visivo da riavviare</h2><p>Il percorso precedente è in stato {progress.track.status} e non può proseguire.</p><button type="button" disabled={busy} onClick={() => setProgress({ ...progress, track: null })}>Scegli un nuovo tratto</button></section> : null}
