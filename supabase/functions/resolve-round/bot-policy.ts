import {
    ...
    selectBotAction,
    type BotRoundAction,
    type SelectBotActionInput,
} from '../../../shared/game-rules/index.ts'

export type EdgeBotRoundAction = BotRoundAction
export type SelectEdgeBotActionInput = {
    traits: SelectBotActionInput['adaptations']
    combatMutationState: SelectBotActionInput['combatMutationState']
    ...
    publicOpponentCombatMutationLoadout: SelectBotActionInput['publicOpponentCombatMutationLoadout']
    symbiosisLinks?: SelectBotActionInput['symbiosisLinks']
    nextRoundEvent?: SelectBotActionInput['roundEvent'] | null
    random?: () => number
    difficulty?: 'EASY' | 'NORMAL' | 'HARD'
}
/** Edge adapter intentionally delegates to the shared policy source of truth. */
export function selectEdgeBotAction({
    traits,
    combatMutationState,
    ...
    random,
    difficulty = 'NORMAL',
}: SelectEdgeBotActionInput): EdgeBotRoundAction {
    ...
    if (difficulty === 'HARD')
        return createLookaheadPolicy({ depth: 2 }).selectAction({
            adaptations: traits,
            combatMutationState,
            combatMutationLoadout,
            roundEvent,
