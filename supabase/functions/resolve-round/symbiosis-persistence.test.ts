...
import { RULE_VERSION, buildPersistedRoundResolution, createInitialAdaptations, createInitialCombatMutationState, getRoundEventById, type CombatMutationLoadout, type SymbiosisLink } from '../../../shared/game-rules/index.ts'
...
const link: SymbiosisLink = { ownerPlayerId: 'p1', sourceTrait: 'FEROCITY', targetPlayerId: 'p2', targetTrait: 'ARMOR', activatedRound: 1 }
...
describe('SYMBIOSIS persistence contract', () => {
    it('persists readable activation data plus before/after match links for reconnect', () => {
        const result = buildPersistedRoundResolution({
            ...
            player1CombatMutationLoadout: loadout, player2CombatMutationLoadout: loadout,
            ...
            player2Action: { playerId: 'p2', trait: 'SENSES', actionType: 'USE' }, priorRoundValues: [], startedAt: null,
        })
        expect(result.resolution_data.symbiosisLinksBefore).toEqual([])
        expect(result.resolution_data.symbiosisLinksAfter).toEqual([link])
        expect(result.resolution_data.player1Action).toMatchObject({ actionType: 'ACTIVATE_MUTATION', mutationId: 'SYMBIOSIS', sourceTrait: 'FEROCITY', targetTrait: 'ARMOR' })
    ...
    it('makes the database own validation, secrecy-compatible payload storage, and atomic link commit', () => {
        ...
        expect(readFileSync(resolve('supabase/migrations/202608080001_game_snapshot_sync.sql'), 'utf8')).toContain("'myCurrentAction', case when v_my_action.id is null")
    })
})
