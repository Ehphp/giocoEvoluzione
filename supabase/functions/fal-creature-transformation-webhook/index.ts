...
async function restoreTrack(
    ...
    record: { profileId: string; id: string; visualProgressTrackId: string | null },
) {
    ...
    try {
        ...
        console.error('fal.webhook.track_restore_failed', {
...
async function invokeFinalizer(input: {
    supabaseUrl: string
    secret: string
    providerRequestId: string
    image: { url: string; contentType: 'image/png' | 'image/jpeg' | null }
}) {
    ...
    if (event.status === 'ERROR') {
        try {
            await repository.markFailed({
                ...
                errorMessage:
                    event.status === 'ERROR'
                        ? 'Fal ha segnalato un errore durante la generazione.'
                        : 'Il webhook Fal non contiene un output immagine valido.',
            })
        } catch {
            return json(200)
        }
