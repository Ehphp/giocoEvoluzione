...
function recordFor(
    input: ReserveCreatureTransformationRequestInput,
    id: string,
    now: string,
): CreatureTransformationRequestRecord {
    return {
        ...
        status: 'RESERVED',
        conceptMode: input.conceptMode ?? null,
        imageProviderMode: input.imageProviderMode ?? null,
        provider: null,
        model: null,
        ...
        generationProfileId: input.generationProfileId ?? null,
        conceptSeed: input.conceptSeed ?? null,
        promptSha256: null,
        promptText: null,
        generationQuality: null,
        visualProgressTrackId: input.visualProgressTrackId ?? null,
        sourceVisualVersionId: input.sourceVisualVersionId ?? null,
        evolutionTargetId: input.evolutionTargetId ?? null,
        evolutionFunction: input.evolutionFunction ?? null,
        visualTraitId: input.visualTraitId ?? null,
        intensity: input.intensity ?? null,
        promptTemplateVersion: null,
        conceptSchemaVersion: null,
        sourceSha256: null,
        resultSha256: null,
        resultPath: null,
        resultMimeType: null,
...
function applyTransition(
    ...
    return {
        ...
        providerRequestId: data.providerRequestId ?? record.providerRequestId,
        promptTemplateVersion: data.promptTemplateVersion ?? record.promptTemplateVersion,
        conceptSchemaVersion: data.conceptSchemaVersion ?? record.conceptSchemaVersion,
        promptSha256: data.promptSha256 ?? record.promptSha256,
        promptText: data.promptText ?? record.promptText,
        sourceSha256: data.sourceSha256 ?? record.sourceSha256,
        resultSha256: data.resultSha256 ?? record.resultSha256,
        resultPath: data.resultPath ?? record.resultPath,
        resultMimeType: data.resultMimeType ?? record.resultMimeType,
        resultWidth: data.resultWidth ?? record.resultWidth,
        resultHeight: data.resultHeight ?? record.resultHeight,
        generationLatencyMs: data.generationLatencyMs ?? record.generationLatencyMs,
        estimatedCostUsd: data.estimatedCostUsd ?? record.estimatedCostUsd,
        actualCostUsd: data.actualCostUsd ?? record.actualCostUsd,
        assetReadiness: data.assetReadiness ?? record.assetReadiness,
        validationWarnings: data.validationWarnings ?? record.validationWarnings,
        conceptSnapshot: data.conceptSnapshot ?? record.conceptSnapshot,
        attemptCount: status === 'RUNNING' ? record.attemptCount + 1 : record.attemptCount,
        startedAt: status === 'RUNNING' ? now : record.startedAt,
        completedAt: status === 'SUCCEEDED' || status === 'FAILED' ? now : record.completedAt,
        errorCode: status === 'FAILED' ? (data.errorCode ?? 'FAILED') : null,
        errorMessage: status === 'FAILED' ? (data.errorMessage ?? 'failed') : null,
        updatedAt: now,
    }
}
...
export function createInMemoryRequestRepository(options: RepositoryOptions = {}) {
    ...
    const calls = {
        ...
        markSucceeded: 0,
        markFailed: 0,
        recordVisualInspection: 0,
        finalizeBackgroundRemovalCandidate: 0,
    }
    let sequence = 0
    ...
    const repository: CreatureTransformationRequestRepository = {
        ...
        async markRunning(input) {
            ...
            const record = records.get(
                [...records.entries()].find(
                    ([, value]) => value.id === input.requestId && value.profileId === input.profileId,
                )?.[0] ?? '',
            )
        ...
        async markFailed(input) {
            ...
            if (!record || (record.status !== 'RESERVED' && record.status !== 'RUNNING'))
        ...
        async updateRunningFalSubmission(input) {
            ...
            if (
                ...
                (input.data.expectedProviderRequestId !== undefined &&
                    record.providerRequestId !== input.data.expectedProviderRequestId)
            )
                throw new Error('state conflict')
            const updated: CreatureTransformationRequestRecord = {
                ...record,
        ...
        async claimFalFinalization(input) {
            const entry = [...records.entries()].find(
                ([, value]) => value.providerRequestId === input.providerRequestId,
            )
            const record = entry?.[1]
            ...
            const updated: CreatureTransformationRequestRecord = {
                ...record,
                falFinalizationRequestId: input.providerRequestId,
                falFinalizationStartedAt: now(),
                updatedAt: now(),
            }
            records.set(entry![0], updated)
        ...
        async finalizeBackgroundRemovalCandidate(input) {
            ...
            const record = entry?.[1]
            if (!record || record.status !== 'SUCCEEDED' || record.assetReadiness !== 'EXPERIMENT_ONLY')
                throw new Error('state conflict')
            const updated: CreatureTransformationRequestRecord = {
                ...record,
                resultPath: input.candidatePath,
                resultSha256: input.candidateSha256,
                resultMimeType: input.candidateMimeType,
                resultWidth: input.candidateWidth,
                resultHeight: input.candidateHeight,
        ...
        async getById(input) {
            return (
                [...records.values()].find(
                    (record) => record.id === input.requestId && record.profileId === input.profileId,
                ) ?? null
            )
        },
        ...
        async getDailyUsage(input) {
            ...
            return {
                ...
                globalRealImageCount: [...records.values()].filter((record) => record.imageProviderMode === 'REAL')
                    .length,
                spentUsd: profileRecords.reduce(
    ...
    return {
        repository,
        calls,
        get(profileId: string, idempotencyKey: string) {
            return records.get(key(profileId, idempotencyKey)) ?? null
        },
        put(record: CreatureTransformationRequestRecord) {
            records.set(key(record.profileId, record.idempotencyKey), record)
        },
    }
}
