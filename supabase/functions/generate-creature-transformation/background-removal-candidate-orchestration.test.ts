...
async function pendingRequest(rawDimensions: readonly [number, number] = [1024, 1536]) {
    ...
    const reserved = await requests.repository.reserve({
        ...
        dailyBudgetUsd: 1,
        visualProgressTrackId: 'track-1',
        sourceVisualVersionId: 'source-1',
    })
    ...
    await requests.repository.markSucceeded({
        ...
        data: {
            resultPath:
                'experiments/raw/profile-1/aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa.png',
            resultSha256: 'a'.repeat(64),
            resultMimeType: 'image/png',
            resultWidth: rawDimensions[0],
            resultHeight: rawDimensions[1],
...
function input(
    ...
    repository: Awaited<ReturnType<typeof pendingRequest>>['requests']['repository'],
    validator: { validate: () => Promise<unknown> },
) {
    ...
    return {
        ...
        body: {
            ...
            candidatePngBase64: btoa('candidate'),
        },
        repository,
        storage: {
            async createCandidateObjectPath() {
                return 'candidates/profile-1/bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb.png'
            },
            async saveBackgroundRemovalCandidate({ image }: { image: Uint8Array }) {
                uploads.push(image)
...
describe('browser background removal candidate gate', () => {
    it('promotes one valid transparent candidate and never invokes OpenAI', async () => {
        ...
        await expect(orchestrateSubmitBackgroundRemovalCandidate(prepared as never)).resolves.toMatchObject({
            success: true,
            candidate: { assetReadiness: 'FINAL_ASSET' },
        })
        expect(prepared.uploads).toHaveLength(1)
        expect(pending.requests.get(profileId, 'raw-key')).toMatchObject({
    ...
    it('returns the existing final asset when a duplicate candidate arrives after the first submission', async () => {
        ...
        await expect(orchestrateSubmitBackgroundRemovalCandidate(prepared as never)).resolves.toMatchObject({
            success: true,
            requestPersistence: { idempotencyStatus: 'EXISTING' },
        })
        expect(prepared.uploads).toHaveLength(1)
    })
    ...
    it('accepts a FLUX raw PNG at 768 by 1152 before promoting the normalized master', async () => {
        ...
        await expect(orchestrateSubmitBackgroundRemovalCandidate(prepared as never)).resolves.toMatchObject({
            success: true,
            candidate: { assetReadiness: 'FINAL_ASSET', width: 1024, height: 1536 },
        })
        expect(pending.requests.get(profileId, 'raw-key')).toMatchObject({
            resultWidth: 1024,
            resultHeight: 1536,
    ...
    it('rejects an opaque candidate before upload and leaves the raw request retryable', async () => {
        ...
        const prepared = input(pending.transformationRequestId, pending.requests.repository, {
            async validate() {
                return { valid: false as const, problems: [{ code: 'PNG_ALPHA_REQUIRED', message: 'alpha required' }] }
            },
        })
        await expect(orchestrateSubmitBackgroundRemovalCandidate(prepared as never)).resolves.toMatchObject({
            success: false,
            code: 'BACKGROUND_REMOVAL_CANDIDATE_INVALID',
        })
