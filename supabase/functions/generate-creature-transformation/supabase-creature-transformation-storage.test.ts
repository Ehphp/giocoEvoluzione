...
function createStorageClient(
    ...
    const download = vi.fn(async () => ({
        data: options.source === null ? null : new Blob([options.source ?? createTestPng()], { type: 'image/png' }),
        error: null,
    }))
    const upload = vi.fn(async () => ({ error: options.uploadError ?? null }))
    const createSignedUrl = vi.fn(async () => ({
        data: options.signedError ? null : { signedUrl: 'https://signed.example/result' },
...
describe('SupabaseCreatureTransformationStorageAdapter', () => {
    it('reads the private source and stores only a deterministic user result with a signed URL', async () => {
        ...
        const stored = await adapter.saveResult({
            profileId: 'profile-1',
    ...
    it('maps missing source, upload and signed-url failures without exposing storage internals', async () => {
        await expect(
            new SupabaseCreatureTransformationStorageAdapter(
                createStorageClient({ source: null }).client,
            ).readCanonicalSource('missing.png'),
        ).rejects.toMatchObject({
            code: 'SOURCE_IMAGE_NOT_FOUND',
        } satisfies Partial<CreatureTransformationStorageError>)
        await expect(
            new SupabaseCreatureTransformationStorageAdapter(
                createStorageClient({ uploadError: { message: 'nope' } }).client,
            ).saveResult({ profileId: 'profile-1', idempotencyKey: 'key', image: createTestPng() }),
        ).rejects.toMatchObject({ code: 'STORAGE_UPLOAD_FAILED' } satisfies Partial<CreatureTransformationStorageError>)
        await expect(
            new SupabaseCreatureTransformationStorageAdapter(
                createStorageClient({ signedError: { message: 'nope' } }).client,
            ).saveResult({ profileId: 'profile-1', idempotencyKey: 'key', image: createTestPng() }),
    ...
    it('keeps lineage sources compatible with legacy server-owned experiment paths', async () => {
        ...
        await expect(adapter.readExperimentalSource('untrusted-source.png')).rejects.toMatchObject({
            code: 'SOURCE_IMAGE_NOT_FOUND',
        } satisfies Partial<CreatureTransformationStorageError>)
        expect(mock.client.from).toHaveBeenCalledWith(CREATURE_TRANSFORMATION_EXPERIMENT_BUCKET)
    ...
    it('signs a constrained legacy raw proposal only for the owner review flow', async () => {
        ...
        await expect(
            adapter.createVisualVersionSignedUrl({ assetPath: legacyPath, isBaseVersion: false }),
        ).resolves.toEqual({ signedUrl: 'https://signed.example/result', expiresAt: '1970-01-01T00:02:00.000Z' })
        await expect(
            adapter.createVisualVersionSignedUrl({
                assetPath: `candidates/profile-1/${'b'.repeat(64)}.png`,
                isBaseVersion: false,
            }),
        ).resolves.toEqual({ signedUrl: 'https://signed.example/result', expiresAt: '1970-01-01T00:02:00.000Z' })
        await expect(
            adapter.createVisualVersionSignedUrl({
                assetPath: 'experiments/raw/profile-1/not-a-hash.png',
                isBaseVersion: false,
            }),
        ).rejects.toMatchObject({ code: 'SIGNED_URL_FAILED' } satisfies Partial<CreatureTransformationStorageError>)
    })
