...
describe('getSafeDatabaseLookupCode', () => {
    it('keeps only a bounded technical database code', () => {
        expect(getSafeDatabaseLookupCode({ code: 'PGRST205', message: 'details that must not be logged' })).toBe(
            'PGRST205',
        )
    ...
    it('does not derive diagnostics from messages or untrusted values', () => {
        expect(getSafeDatabaseLookupCode({ message: 'invalid input syntax for type uuid: secret-value' })).toBe(
            'UNKNOWN',
        )
