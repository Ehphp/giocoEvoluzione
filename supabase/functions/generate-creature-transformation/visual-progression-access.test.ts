...
import {
    ...
    type SupabaseCreatureVisualProgressionRepository,
} from './creature-visual-progression-repository.ts'
import {
    orchestrateGenerateUnlockedTransformation,
    orchestrateGetCreatureVisualProgress,
    orchestrateGetCurrentCreatureVisual,
    orchestrateGetGameCreatureVisuals,
...
function version(
    profileId: string,
    creatureId: string,
    visualTraitId: StoredVisualVersion['visualTraitId'] = null,
): StoredVisualVersion {
    return {
        id: `${profileId}-${creatureId}`,
        creatureId,
        profileId,
        versionNumber: visualTraitId ? 2 : 1,
        previousVersionId: null,
        visualTraitId,
        conceptName: visualTraitId ? 'Carapace rinforzato' : null,
        conceptSnapshot: null,
        promptTemplateVersion: null,
        promptSha256: null,
        assetPath: 'verdant-hatchling-v1.png',
...
function visualRepository(
    lastFailure: { requestId: string; code: string; message: string } | null = null,
    ...
    return {
        ...
        async listVisualHistory({ profileId, creatureId }) {
            return [version(profileId, creatureId)]
        },
...
describe('visual progression access', () => {
    it('allows every authenticated profile to read its visual progress and current visual', async () => {
        await expect(
            orchestrateGetCreatureVisualProgress(
                input({ operation: 'GET_VISUAL_PROGRESS', creatureId: CREATURE_ID }) as never,
            ),
        ).resolves.toMatchObject({ success: true, currentVersion: { versionNumber: 1 }, bodyPlan: { id: 'QUADRUPED' } })
        await expect(
            orchestrateGetCurrentCreatureVisual(
                input({ operation: 'GET_CURRENT_VISUAL', creatureId: CREATURE_ID }) as never,
            ),
        ).resolves.toMatchObject({
            success: true,
            visual: { signedUrl: 'https://signed.example/verdant-hatchling-v1.png' },
        })
    })

    it('returns the current version short description from its persisted visual inspection', async () => {
        const shortDescription =
            'Una creatura quadrupede dalle scaglie verdi, con una coda lunga e luminose placche dorsali.'
        const repository = {
            ...visualRepository(),
            async getCurrentVersion({ profileId, creatureId }: { profileId: string; creatureId: string }) {
                return {
                    ...version(profileId, creatureId),
                    visualInspection: inspectionWithDescription(shortDescription),
                }
            },
        } as unknown as SupabaseCreatureVisualProgressionRepository
        ...
        await expect(
            orchestrateGetCreatureVisualProgress({
                ...input({ operation: 'GET_VISUAL_PROGRESS', creatureId: CREATURE_ID }),
    ...
    it('reads the short description from an active visual-version database record', () => {
        ...
        const mapped = mapVisualVersion({
            ...
            evolution_target_id: 'DORSAL_STRUCTURES',
            evolution_function: 'DEFENSE',
            concept_name: 'Placche luminose',
            concept_snapshot: null,
            prompt_template_version: null,
            prompt_sha256: null,
            asset_path: 'cleanup/' + 'a'.repeat(64) + '.png',
            ...
            has_alpha: true,
            status: 'ACTIVE',
            adopted_at: '2026-08-17T00:00:00.000Z',
    ...
    it('prefers a persisted WebP display asset and falls back to the legacy PNG master', async () => {
        const displayVersion = {
            ...
            displayMimeType: 'image/webp' as const,
            displayWidth: 512,
            displayHeight: 768,
        }
        ...
        await expect(
            orchestrateGetCurrentCreatureVisual({
                ...input({ operation: 'GET_CURRENT_VISUAL', creatureId: CREATURE_ID }),
                visualRepository: displayRepository,
            } as never),
        ).resolves.toMatchObject({
            success: true,
            visual: {
                signedUrl: `https://signed.example/display/${'a'.repeat(64)}.webp`,
                mimeType: 'image/webp',
                width: 512,
                height: 768,
            },
        ...
        await expect(
            ...
            success: true,
            visual: { signedUrl: 'https://signed.example/verdant-hatchling-v1.png', mimeType: 'image/png' },
        })
    })
    ...
    it('returns the latest failed generation for an otherwise ready track', async () => {
        const failure = {
            ...
            code: 'FAL_FLUX_PROVIDER_ERROR',
            message: 'Il provider immagini non ha completato la richiesta.',
        }
        await expect(
            orchestrateGetCreatureVisualProgress({
                ...input({ operation: 'GET_VISUAL_PROGRESS', creatureId: CREATURE_ID }),
                visualRepository: visualRepository(failure),
            } as never),
    ...
    it('hides an older failure while a newer generation is being post-processed', async () => {
        const failure = {
            requestId: 'failed-request',
            code: 'FLUX_SUBJECT_CROPPED',
            message: 'Il soggetto era troppo vicino al bordo.',
        }
        const repository = {
            ...
            async getTrack() {
                return {
                    ...
                    progress: 1,
                    target: 1,
                    readyAt: '2026-08-15T13:15:43.125834+00:00',
                    generatedRequestId: 'new-request',
                    completedVersionId: null,
                }
            },
        } as unknown as SupabaseCreatureVisualProgressionRepository
    ...
    it('shares the opponent current visual only with a participant of that game', async () => {
        await expect(
            orchestrateGetGameCreatureVisuals(
                input({ operation: 'GET_GAME_VISUALS', gameId: '00000000-0000-4000-8000-000000000003' }) as never,
            ),
        ...
        await expect(
            orchestrateGenerateUnlockedTransformation(
                input({
                    ...
                    creatureId: CREATURE_ID,
                    progressTrackId: '00000000-0000-4000-8000-000000000004',
                    idempotencyKey: 'friend-attempt',
                }) as never,
            ),
    ...
    it('refuses an unlocked generation while the FLUX pipeline is not configured', async () => {
        ...
        ).resolves.toMatchObject({ success: false, code: 'FAL_FLUX_NOT_CONFIGURED' })
    })
})
