...
describe('request persistence state contract', () => {
    it('returns CREATED once per profile/key and permits the same key for another profile', async () => {
        ...
        const first = await requests.repository.reserve({
            ...reserveInput,
            profileId: 'profile-1',
            idempotencyKey: 'same-key',
        })
        const repeated = await requests.repository.reserve({
            ...reserveInput,
            profileId: 'profile-1',
            idempotencyKey: 'same-key',
        })
        const anotherProfile = await requests.repository.reserve({
            ...reserveInput,
            profileId: 'profile-2',
            idempotencyKey: 'same-key',
        })
        ...
        expect(repeated).toMatchObject({
            outcome: 'EXISTING',
            record: { idempotencyKey: 'same-key', profileId: 'profile-1' },
        })
    ...
    it('allows only RESERVED -> RUNNING -> SUCCEEDED and maintains audit timestamps', async () => {
        ...
        await expect(
            requests.repository.markRunning({ requestId: reserved.record.id, profileId: 'profile-1' }),
        ).rejects.toThrow('state conflict')
        await expect(
            requests.repository.markFailed({
                requestId: reserved.record.id,
                profileId: 'profile-1',
                errorCode: 'FAIL',
                errorMessage: 'fail',
    ...
    it('retains reserved target direction metadata after a failed request', async () => {
        ...
        const reservation = await persistence.repository.reserve({
            profileId: 'profile-1',
            creatureId: 'creature-1',
            idempotencyKey: 'target-failed',
            operation: 'GENERATE_UNLOCKED_TRANSFORMATION',
            visualTraitId: 'ENERGY_REGULATION',
            evolutionTargetId: 'TORSO_AND_BACK',
            evolutionFunction: 'THERMOREGULATION',
            intensity: 2,
            conceptMode: 'AI',
        ...
        const running = await persistence.repository.markRunning({
