...
describe('creature evolution policy', () => {
    ...
    it('keeps the body-plan mutation capability disabled unless it is explicitly enabled', () => {
        ...
        expect(readCreatureEvolutionPolicy((name) => ({ CREATURE_EVOLUTION_BODY_PLAN_MUTATION_ENABLED: 'yes' })[name]).bodyPlanMutation.enabled).toBe(false)
        expect(readCreatureEvolutionPolicy((name) => ({ CREATURE_EVOLUTION_BODY_PLAN_MUTATION_ENABLED: 'true' })[name]).bodyPlanMutation.enabled).toBe(true)
    })
    ...
    it('reads the server-only provider configuration and the access lists without trusting client input', () => {
        ...
        expect(policy.seedream).toMatchObject({ apiKey: 'server-only-fal-key', timeoutMs: 45000, estimatedCostUsd: 0.07, maxEstimatedCostUsd: 0.08 })
        expect(policy.microConcept).toEqual({ apiKey: 'server-only-concept-key', model: 'configured-micro-concept-model' })
    ...
    it('enables Seedream structural mutations only through its own explicit flag', () => {
        ...
        expect(readCreatureEvolutionPolicy((name) => ({ SEEDREAM_STRUCTURAL_MUTATIONS_ENABLED: 'true' })[name]).seedream.structuralMutationsEnabled).toBe(true)
    })
