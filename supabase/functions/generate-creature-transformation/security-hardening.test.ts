...
const migration = readFileSync(
    resolve(process.cwd(), 'supabase/migrations/202608050001_security_hardening.sql'),
    'utf8',
)
...
const webhook = readFileSync(
    resolve(process.cwd(), 'supabase/functions/fal-creature-transformation-webhook/index.ts'),
    'utf8',
)
const finalizer = readFileSync(
    resolve(process.cwd(), 'supabase/functions/fal-creature-transformation-finalizer/index.ts'),
    'utf8',
)
...
describe('security hardening regression controls', () => {
    it('disables public signup, keeps user routes on JWT and makes Fal callbacks explicit exceptions', () => {
        ...
        expect([...config.matchAll(/^enable_signup\s*=\s*(\w+)/gm)].map((match) => match[1])).toEqual([
            'false',
            'false',
            'false',
        ])
    ...
    it('keeps provider credentials out of client configuration and public error paths', () => {
        ...
        expect(envExample).not.toMatch(
            /^VITE_.*(?:FAL|OPENAI|TOGETHER|GEMINI|SERVICE_ROLE|DATABASE|JWT|SECRET|TOKEN|PASSWORD)/m,
        )
        expect(transformationClient).not.toMatch(
            /fal\.ai|FAL_[A-Z_]*API_KEY|OPENAI_API_KEY|TOGETHER_API_KEY|GEMINI_API_KEY|SUPABASE_SERVICE_ROLE_KEY/,
        )
