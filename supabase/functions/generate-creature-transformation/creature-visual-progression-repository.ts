...
type Query = PromiseLike<{ data: unknown; error: DatabaseError }> & {
...
function rpcRecord(value: unknown): Record<string, unknown> {
    ...
    if (!result)
        throw new CreatureVisualProgressionRepositoryError(
            ...
            'La risposta persistente della progressione non e valida.',
        )
    return result
}
...
function safeVisualErrorCode(error: unknown, fallback: string): string {
    ...
    const code = message.match(
        /\b(CREATURE_NOT_OWNED|VISUAL_TRACK_ALREADY_ACTIVE|VISUAL_TRACK_NOT_FOUND|VISUAL_TRACK_NOT_READY|VISUAL_TRACK_STATE_CONFLICT|VISUAL_TRAIT_INVALID|EVOLUTION_TARGET_INVALID|VISUAL_GENERATION_NOT_ADOPTABLE|VISUAL_VERSION_NOT_FOUND|CREATURE_VISUAL_VERSION_CONFLICT|CREATURE_VISUAL_ALREADY_ADOPTED|VISUAL_ROLLBACK_FAILED|BACKGROUND_CLEANUP_VERSION_CONFLICT|BACKGROUND_CLEANUP_CANDIDATE_INVALID)\b/,
    )?.[1]
    return code ?? fallback
}

function visualProgressionErrorMessage(code: string): string {
    if (code === 'VISUAL_TRACK_ALREADY_ACTIVE')
        return 'Esiste gia un percorso visuale aperto. Il suo stato e stato ricaricato.'
    ...
    if (code === 'CREATURE_VISUAL_VERSION_CONFLICT')
        return 'La versione visuale e cambiata. Ricarica il percorso e riprova.'
...
export function mapProgressTrack(value: unknown): CreatureVisualProgressTrack {
    ...
    if (
        !row ||
        !string(row.id) ||
        !string(row.creature_id) ||
        (!string(row.visual_trait_id) && !string(row.evolution_target_id))
    )
        throw new CreatureVisualProgressionRepositoryError(
            'VISUAL_TRACK_STATE_CONFLICT',
            'La track visuale restituita non e valida.',
        )
    const status = string(row.status)
    if (
        !status ||
        !['ACTIVE', 'READY', 'GENERATING', 'POST_PROCESSING', 'GENERATED', 'COMPLETED', 'CANCELLED'].includes(status)
    )
        throw new CreatureVisualProgressionRepositoryError(
            'VISUAL_TRACK_STATE_CONFLICT',
            'Lo stato della track visuale non e valido.',
        )
    return {
        id: string(row.id)!,
        creatureId: string(row.creature_id)!,
        visualTraitId: nullableString(row.visual_trait_id) as VisualTraitId | null,
        evolutionTargetId: nullableString(row.evolution_target_id) as EvolutionTargetId | null,
        status: status as CreatureVisualProgressTrack['status'],
        progress: number(row.progress),
        target: number(row.target),
        readyAt: nullableString(row.ready_at),
        generatedRequestId: nullableString(row.generated_request_id),
        completedVersionId: nullableString(row.completed_version_id),
    }
}
...
export function mapVisualVersion(value: unknown): StoredVisualVersion {
    ...
    if (
        ...
        !string(row.asset_sha256)
    )
        throw new CreatureVisualProgressionRepositoryError(
            'VISUAL_VERSION_NOT_FOUND',
            'La versione visuale restituita non e valida.',
        )
    const status = string(row.status)
    if (!status || !['BASE', 'ACTIVE', 'SUPERSEDED', 'REVOKED'].includes(status))
        throw new CreatureVisualProgressionRepositoryError(
            'VISUAL_VERSION_NOT_FOUND',
            'Lo stato della versione visuale non e valido.',
        )
    return {
        id: string(row.id)!,
        creatureId: string(row.creature_id)!,
        ...
        conceptName: nullableString(row.concept_name),
        conceptSnapshot:
            row.concept_snapshot && typeof row.concept_snapshot === 'object'
                ? (row.concept_snapshot as StoredVisualVersion['conceptSnapshot'])
                : null,
        promptTemplateVersion: nullableString(row.prompt_template_version),
        ...
        mimeType: row.mime_type === 'image/png' ? 'image/png' : 'image/png',
        width: number(row.width),
        height: number(row.height),
        hasAlpha: row.has_alpha === true,
        status: status as StoredVisualVersion['status'],
        adoptedAt: nullableString(row.adopted_at),
        displayAssetPath: nullableString(row.display_asset_path),
        displayAssetSha256: nullableString(row.display_asset_sha256),
        displayMimeType: row.display_mime_type === 'image/webp' ? 'image/webp' : null,
        displayWidth: row.display_width === null || row.display_width === undefined ? null : number(row.display_width),
        displayHeight:
            row.display_height === null || row.display_height === undefined ? null : number(row.display_height),
    }
}
...
export class SupabaseCreatureVisualProgressionRepository {
    ...
    async getTrack(input: { profileId: string; creatureId: string }): Promise<CreatureVisualProgressTrack | null> {
        const { data, error } = await this.client
            .from('creature_visual_progress_tracks')
            .select('*')
        ...
        if (error)
            throw new CreatureVisualProgressionRepositoryError(
                'VISUAL_TRACK_STATE_CONFLICT',
                'Impossibile recuperare la track visuale.',
                { cause: error },
            )
        return data ? mapProgressTrack(data) : null
    }

    async getLatestExperiment(input: {
        profileId: string
        trackId: string
    }): Promise<StoredExperimentOnlyResult | null> {
        const { data, error } = await this.client
            .from('creature_transformation_requests')
            .select('id, asset_readiness, validation_warnings')
            .eq('profile_id', input.profileId)
            .eq('visual_progress_track_id', input.trackId)
            .eq('status', 'SUCCEEDED')
            .eq('asset_readiness', 'EXPERIMENT_ONLY')
        ...
        if (error)
            throw new CreatureVisualProgressionRepositoryError(
                'VISUAL_TRACK_STATE_CONFLICT',
                'Impossibile recuperare l esito sperimentale.',
                { cause: error },
            )
    ...
    async getLatestFailure(input: {
        profileId: string
        trackId: string
    }): Promise<StoredVisualGenerationFailure | null> {
        const { data, error } = await this.client
            .from('creature_transformation_requests')
        ...
        if (error)
            throw new CreatureVisualProgressionRepositoryError(
                'VISUAL_TRACK_STATE_CONFLICT',
                'Impossibile recuperare l errore di generazione.',
                { cause: error },
            )
        const row = record(data)
    ...
    async getCurrentVersion(input: { profileId: string; creatureId: string }): Promise<StoredVisualVersion | null> {
        ...
        if (error)
            throw new CreatureVisualProgressionRepositoryError(
                'CURRENT_VISUAL_UNAVAILABLE',
                'Impossibile recuperare la visuale corrente.',
                { cause: error },
            )
    ...
    async listGameHumanParticipants(gameId: string): Promise<GameVisualParticipant[]> {
        const { data, error } = await this.client
            .from('players')
            .select('profile_id, creature_id, player_type')
            .eq('game_id', gameId)
        if (error)
            throw new CreatureVisualProgressionRepositoryError(
                'OPPONENT_VISUAL_NOT_AUTHORIZED',
                'Impossibile recuperare i partecipanti alla partita.',
                { cause: error },
            )
    ...
    async startGeneration(input: {
        ...
        return mapProgressTrack(
            await this.rpc('start_creature_visual_generation', {
    ...
    async resolveTrackTrait(input: {
        ...
        return mapProgressTrack(
            await this.rpc('resolve_creature_visual_progress_track_trait', {
                p_profile_id: input.profileId,
                p_creature_id: input.creatureId,
                p_track_id: input.trackId,
                p_visual_trait_id: input.visualTraitId,
            }),
        )
    }
    ...
    async completeGeneration(input: {
        ...
        return mapProgressTrack(
            await this.rpc('complete_creature_visual_generation', {
                p_profile_id: input.profileId,
                p_track_id: input.trackId,
                p_request_id: input.requestId,
                p_final_asset: input.finalAsset,
            }),
        )
    }

    async markBackgroundRemovalPending(input: {
        profileId: string
        trackId: string
        requestId: string
    }): Promise<CreatureVisualProgressTrack> {
        return mapProgressTrack(
            await this.rpc('mark_creature_visual_background_removal_pending', {
                p_profile_id: input.profileId,
                p_track_id: input.trackId,
                p_request_id: input.requestId,
            }),
    ...
    async restoreNonFinalGeneration(input: {
        profileId: string
        trackId: string
        requestId: string
    }): Promise<CreatureVisualProgressTrack> {
        return mapProgressTrack(
            await this.rpc('restore_nonfinal_creature_visual_generation', {
                p_profile_id: input.profileId,
                p_track_id: input.trackId,
                p_request_id: input.requestId,
            }),
    ...
    async adopt(input: {
        ...
        return mapVisualVersion(
            await this.rpc('adopt_creature_transformation', {
                p_profile_id: input.profileId,
                p_creature_id: input.creatureId,
    ...
    async rollback(input: {
        ...
        return mapVisualVersion(
            await this.rpc('rollback_creature_visual_version', {
                p_profile_id: input.profileId,
                p_creature_id: input.creatureId,
    ...
    private async rpc(name: string, args: Record<string, unknown>): Promise<Record<string, unknown>> {
        try {
            ...
            throw new CreatureVisualProgressionRepositoryError(code, visualProgressionErrorMessage(code), {
                cause: error,
            })
        }
    }
