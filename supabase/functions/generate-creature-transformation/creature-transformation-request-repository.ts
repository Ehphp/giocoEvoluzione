...
export type CreatureTransformationRequestOperation =
    'GENERATE_CONCEPT' | 'GENERATE_IMAGE' | 'GENERATE_UNLOCKED_TRANSFORMATION'
...
export type RequestReservationOutcome =
    | 'CREATED'
    | 'EXISTING'
    | 'DAILY_LIMIT_REACHED'
    ...
    | 'REAL_IMAGE_USER_LIMIT_REACHED'
    | 'REAL_IMAGE_USER_CONCURRENCY_REACHED'
    | 'REAL_IMAGE_COOLDOWN_ACTIVE'
    | 'REAL_IMAGE_GLOBAL_LIMIT_REACHED'
    | 'REAL_IMAGE_GLOBAL_CONCURRENCY_REACHED'
/**
...
export type ReserveCreatureTransformationRequestInput = TransformationCost &
    Readonly<{
        profileId: string
        creatureId: string
        idempotencyKey: string
...
export type RequestTransitionData = TransformationCost &
    Readonly<{
        provider?: string
        model?: string
...
export interface CreatureTransformationRequestRepository {
    ...
    }): Promise<CreatureTransformationRequestRecord>
    recordVisualInspection(input: {
        requestId: string
        profileId: string
    ...
    updateRunningFalSubmission(input: {
        requestId: string
        profileId: string
        data: RunningFalSubmissionData
    }): Promise<CreatureTransformationRequestRecord>
    claimFalFinalization(input: { providerRequestId: string }): Promise<FalFinalizationClaim>
    finalizeBackgroundRemovalCandidate(input: {
        requestId: string
        profileId: string
        ...
        validationWarnings: string[]
        displayAsset?: { path: string; sha256: string; width: number; height: number }
    }): Promise<CreatureTransformationRequestRecord>
    getByIdempotencyKey(input: {
        profileId: string
        idempotencyKey: string
    }): Promise<CreatureTransformationRequestRecord | null>
    ...
    listCompletedImageRecords(input: {
        profileId: string
        offset: number
        limit: number
    }): Promise<CreatureTransformationRequestRecord[]>
...
export type CreatureTransformationRequestRepositoryErrorCode =
    'REQUEST_RESERVATION_FAILED' | 'REQUEST_STATE_CONFLICT' | 'REQUEST_PERSISTENCE_FAILED'
...
export class CreatureTransformationRequestRepositoryError extends Error {
    ...
    constructor(
        code: CreatureTransformationRequestRepositoryErrorCode,
        message: string,
...
function mapRecord(value: unknown): CreatureTransformationRequestRecord {
    ...
    if (!record)
        throw new CreatureTransformationRequestRepositoryError(
            'REQUEST_PERSISTENCE_FAILED',
            'Il record persistente restituito non e valido.',
        )
    const required = [
        'id',
        'profile_id',
    ...
    if (required.some((field) => !readString(record, field))) {
        throw new CreatureTransformationRequestRepositoryError(
            'REQUEST_PERSISTENCE_FAILED',
            'Il record persistente restituito non e completo.',
        )
    }
    ...
    if (!['RESERVED', 'RUNNING', 'SUCCEEDED', 'FAILED'].includes(status)) {
        throw new CreatureTransformationRequestRepositoryError(
            'REQUEST_PERSISTENCE_FAILED',
            'Lo stato persistente restituito non e supportato.',
        )
    }
    ...
    if (
        ...
        throw new CreatureTransformationRequestRepositoryError(
            'REQUEST_PERSISTENCE_FAILED',
            'L operazione persistente restituita non e supportata.',
        )
    }
    ...
    return {
        ...
        creatureId: readString(record, 'creature_id')!,
        idempotencyKey: readString(record, 'idempotency_key')!,
        operation,
        status,
        ...
        benchmarkCaseId: readString(record, 'benchmark_case_id', true),
        generationProfileId: readString(record, 'generation_profile_id', true),
        conceptSeed: readString(record, 'concept_seed', true),
        promptSha256: readString(record, 'prompt_sha256', true),
        promptText: readString(record, 'prompt_text', true),
        generationQuality: readString(record, 'generation_quality', true) as 'low' | 'medium' | 'high' | null,
        visualProgressTrackId: readString(record, 'visual_progress_track_id', true),
        sourceVisualVersionId: readString(record, 'source_visual_version_id', true),
        ...
        provider: readString(record, 'provider', true),
        model: readString(record, 'model', true),
        providerRequestId: readString(record, 'provider_request_id', true),
        falWorkflow: asRecord(record.fal_workflow),
        falFinalizationRequestId: readString(record, 'fal_finalization_request_id', true),
        falFinalizationStartedAt: readString(record, 'fal_finalization_started_at', true),
        visualTraitId: readString(record, 'visual_trait_id', true),
        intensity: readNumber(record, 'intensity', true),
        promptTemplateVersion: readString(record, 'prompt_template_version', true),
        conceptSchemaVersion: readNumber(record, 'concept_schema_version', true),
        sourceSha256: readString(record, 'source_sha256', true),
        resultSha256: readString(record, 'result_sha256', true),
        resultPath: readString(record, 'result_path', true),
        resultMimeType: readString(record, 'result_mime_type', true) as 'image/png' | 'image/jpeg' | null,
        resultWidth: readNumber(record, 'result_width', true),
        resultHeight: readNumber(record, 'result_height', true),
        generationLatencyMs: readNumber(record, 'generation_latency_ms', true),
        estimatedCostUsd: readNumber(record, 'estimated_cost_usd', true),
        actualCostUsd: readNumber(record, 'actual_cost_usd', true),
        assetReadiness: readString(record, 'asset_readiness', true) as CreatureTransformationAssetReadiness | null,
        validationWarnings: readWarnings(record),
        conceptSnapshot:
            record.concept_snapshot && typeof record.concept_snapshot === 'object'
                ? (record.concept_snapshot as CreatureTransformationConceptSnapshot)
                : null,
        visualInspection: parseVisualInspection(record.visual_inspection),
        attemptCount: readNumber(record, 'attempt_count') ?? 0,
        errorCode: readString(record, 'error_code', true),
        errorMessage: readString(record, 'error_message', true),
        createdAt: readString(record, 'created_at')!,
        startedAt: readString(record, 'started_at', true),
        completedAt: readString(record, 'completed_at', true),
...
function readRpcResult(value: unknown): { outcome: string; record?: CreatureTransformationRequestRecord } {
    ...
    if (!outcome)
        throw new CreatureTransformationRequestRepositoryError(
            'REQUEST_PERSISTENCE_FAILED',
            'La RPC di persistenza ha restituito una risposta non valida.',
        )
...
export class SupabaseCreatureTransformationRequestRepository implements CreatureTransformationRequestRepository {
    ...
    async reserve(input: ReserveCreatureTransformationRequestInput): Promise<RequestReservationResult> {
        ...
        try {
            response = await this.client.rpc('reserve_creature_transformation_request', {
                ...
                p_visual_trait_id: input.visualTraitId ?? null,
                p_intensity: input.intensity ?? null,
                p_concept_mode: input.conceptMode ?? null,
                p_image_provider_mode: input.imageProviderMode ?? null,
                p_estimated_cost_usd: input.estimatedCostUsd ?? null,
                p_daily_request_limit: input.dailyRequestLimit,
                p_daily_budget_usd: input.dailyBudgetUsd,
                p_benchmark_case_id: input.benchmarkCaseId ?? null,
                p_generation_profile_id: input.generationProfileId ?? null,
                p_concept_seed: input.conceptSeed ?? null,
                p_visual_progress_track_id: input.visualProgressTrackId ?? null,
                p_source_visual_version_id: input.sourceVisualVersionId ?? null,
                p_evolution_target_id: input.evolutionTargetId ?? null,
                p_evolution_function: input.evolutionFunction ?? null,
            ...
            throw new CreatureTransformationRequestRepositoryError(
                ...
                'Non e stato possibile riservare la richiesta.',
                { cause: error },
            )
        }
        if (response.error)
        ...
        if (result.outcome === 'CREATED' || result.outcome === 'EXISTING') {
            if (!result.record)
                throw new CreatureTransformationRequestRepositoryError(
                    'REQUEST_PERSISTENCE_FAILED',
                    'La riserva non contiene il record richiesto.',
                )
            return { outcome: result.outcome, record: result.record }
        }
        if (
            result.outcome === 'DAILY_LIMIT_REACHED' ||
            result.outcome === 'DAILY_BUDGET_REACHED' ||
            result.outcome === 'CREATURE_NOT_OWNED' ||
            result.outcome === 'IDEMPOTENCY_KEY_REUSED' ||
            result.outcome === 'REAL_IMAGE_USER_LIMIT_REACHED' ||
            result.outcome === 'REAL_IMAGE_USER_CONCURRENCY_REACHED' ||
            result.outcome === 'REAL_IMAGE_COOLDOWN_ACTIVE' ||
            result.outcome === 'REAL_IMAGE_GLOBAL_LIMIT_REACHED' ||
            result.outcome === 'REAL_IMAGE_GLOBAL_CONCURRENCY_REACHED'
        )
            return { outcome: result.outcome }
        throw new CreatureTransformationRequestRepositoryError(
            'REQUEST_RESERVATION_FAILED',
            'La riserva ha restituito un esito non supportato.',
        )
    }
    ...
    async recordVisualInspection(input: {
        ...
        try {
            ...
            throw new CreatureTransformationRequestRepositoryError(
                'REQUEST_PERSISTENCE_FAILED',
                'Non e stato possibile salvare l ispezione visuale.',
                { cause: error },
            )
        }
        if (response.error)
        ...
        if (result.outcome === 'CONFLICT')
            throw new CreatureTransformationRequestRepositoryError(
                ...
                'La richiesta non e piu disponibile per l ispezione visuale.',
            )
        if (result.outcome !== 'UPDATED' || !result.record)
            throw new CreatureTransformationRequestRepositoryError(
                'REQUEST_PERSISTENCE_FAILED',
                'L ispezione visuale non ha restituito il record aggiornato.',
            )
    ...
    async updateRunningFalSubmission(input: {
        ...
        try {
            response = await this.client.rpc('update_running_fal_submission', {
                ...
                p_model: data.model,
                p_provider_request_id: data.providerRequestId,
                p_source_sha256: data.sourceSha256 ?? null,
                p_prompt_template_version: data.promptTemplateVersion ?? null,
                p_prompt_sha256: data.promptSha256 ?? null,
                p_prompt_text: data.promptText ?? null,
                p_concept_snapshot: data.conceptSnapshot ?? null,
                p_fal_workflow: data.falWorkflow ?? null,
            ...
            throw new CreatureTransformationRequestRepositoryError(
                'REQUEST_PERSISTENCE_FAILED',
                'Non e stato possibile salvare la submission Fal.',
                { cause: error },
            )
        }
        if (response.error)
        ...
        if (result.outcome === 'CONFLICT')
            throw new CreatureTransformationRequestRepositoryError(
                'REQUEST_STATE_CONFLICT',
                'La richiesta non e piu disponibile per una submission Fal.',
            )
        if (result.outcome !== 'UPDATED' || !result.record)
            throw new CreatureTransformationRequestRepositoryError(
                'REQUEST_PERSISTENCE_FAILED',
                'La submission Fal non ha restituito il record aggiornato.',
            )
        return result.record
    }
    ...
    async claimFalFinalization(input: { providerRequestId: string }): Promise<FalFinalizationClaim> {
        let response: { data: unknown; error: DatabaseError }
        try {
            response = await this.client.rpc('claim_fal_transformation_finalization', {
                p_provider_request_id: input.providerRequestId,
            })
        } catch (error) {
            throw new CreatureTransformationRequestRepositoryError(
                'REQUEST_PERSISTENCE_FAILED',
                'Non e stato possibile acquisire la finalizzazione Fal.',
                { cause: error },
            )
        }
        if (response.error)
        ...
        if (
            (result.outcome === 'CLAIMED' || result.outcome === 'IN_PROGRESS' || result.outcome === 'TERMINAL') &&
            result.record
        )
            return { outcome: result.outcome, record: result.record }
        throw new CreatureTransformationRequestRepositoryError(
            'REQUEST_PERSISTENCE_FAILED',
            'La finalizzazione Fal ha restituito un esito non valido.',
        )
    }
    ...
    async finalizeBackgroundRemovalCandidate(input: {
        ...
        try {
            response = await this.client.rpc('finalize_creature_background_removal_candidate', {
                ...
                p_candidate_mime_type: input.candidateMimeType,
                p_candidate_width: input.candidateWidth,
                p_candidate_height: input.candidateHeight,
                p_validation_warnings: input.validationWarnings,
                p_display_asset_path: input.displayAsset?.path ?? null,
                p_display_asset_sha256: input.displayAsset?.sha256 ?? null,
                p_display_mime_type: input.displayAsset ? 'image/webp' : null,
                p_display_width: input.displayAsset?.width ?? null,
                p_display_height: input.displayAsset?.height ?? null,
            })
        } catch (error) {
            throw new CreatureTransformationRequestRepositoryError(
                'REQUEST_PERSISTENCE_FAILED',
                'Non e stato possibile finalizzare il PNG elaborato.',
                { cause: error },
            )
        }
        if (response.error)
        ...
        if (result.outcome === 'CONFLICT')
            throw new CreatureTransformationRequestRepositoryError(
                'REQUEST_STATE_CONFLICT',
                'La richiesta non e pronta per il PNG elaborato.',
            )
        if (result.outcome !== 'UPDATED' || !result.record)
            throw new CreatureTransformationRequestRepositoryError(
                'REQUEST_PERSISTENCE_FAILED',
                'La finalizzazione non ha restituito il record aggiornato.',
            )
        return result.record
    }
    ...
    async getByProviderRequestId(input: {
        ...
        try {
            response = await this.client
                ...
                .select('*')
                .eq('provider_request_id', input.providerRequestId)
                .maybeSingle()
        } catch (error) {
            throw new CreatureTransformationRequestRepositoryError(
                'REQUEST_PERSISTENCE_FAILED',
                'Non e stato possibile recuperare la richiesta Fal.',
                { cause: error },
            )
        }
        if (response.error)
    ...
    async getDailyUsage(input: { profileId: string }): Promise<CreatureTransformationDailyUsage> {
        let response: { data: unknown; error: DatabaseError }
        try {
            response = await this.client.rpc('get_creature_transformation_daily_usage', {
                p_profile_id: input.profileId,
            })
        } catch (error) {
            throw new CreatureTransformationRequestRepositoryError(
                'REQUEST_PERSISTENCE_FAILED',
                'Non e stato possibile leggere l utilizzo giornaliero.',
                { cause: error },
            )
        }
        if (response.error)
        ...
        if (
            requestCount === null ||
            realImageCount === null ||
            globalRealImageCount === null ||
            spentUsd === null ||
            requestCount < 0 ||
            realImageCount < 0 ||
            globalRealImageCount < 0 ||
            spentUsd < 0
        ) {
            throw new CreatureTransformationRequestRepositoryError(
                'REQUEST_PERSISTENCE_FAILED',
                'L utilizzo giornaliero restituito non e valido.',
            )
        }
    ...
    private async getOne(
        ...
        try {
            ...
            throw new CreatureTransformationRequestRepositoryError(
                'REQUEST_PERSISTENCE_FAILED',
                'Non e stato possibile recuperare la richiesta persistente.',
                { cause: error },
            )
        }
        if (response.error)
    ...
    private async transition(
        input: { requestId: string; profileId: string },
        ...
        try {
            response = await this.client.rpc('transition_creature_transformation_request', {
                ...
                p_prompt_template_version: data.promptTemplateVersion ?? null,
                p_concept_schema_version: data.conceptSchemaVersion ?? null,
                p_source_sha256: data.sourceSha256 ?? null,
                p_result_sha256: data.resultSha256 ?? null,
                p_result_path: data.resultPath ?? null,
                p_result_mime_type: data.resultMimeType ?? null,
                p_result_width: data.resultWidth ?? null,
                p_result_height: data.resultHeight ?? null,
                p_generation_latency_ms: data.generationLatencyMs ?? null,
                p_estimated_cost_usd: data.estimatedCostUsd ?? null,
                p_actual_cost_usd: data.actualCostUsd ?? null,
                p_error_code: data.errorCode ?? null,
                p_error_message: data.errorMessage ?? null,
                p_asset_readiness: data.assetReadiness ?? null,
                p_validation_warnings: data.validationWarnings ?? null,
                p_generation_quality: data.generationQuality ?? null,
                p_prompt_sha256: data.promptSha256 ?? null,
                p_concept_snapshot: data.conceptSnapshot ?? null,
                p_prompt_text: data.promptText ?? null,
            })
            ...
            throw new CreatureTransformationRequestRepositoryError(
                'REQUEST_PERSISTENCE_FAILED',
                'Non e stato possibile aggiornare lo stato della richiesta.',
                { cause: error },
            )
        }
        if (response.error)
        ...
        if (result.outcome === 'CONFLICT')
            throw new CreatureTransformationRequestRepositoryError(
                'REQUEST_STATE_CONFLICT',
                'La richiesta e in uno stato non compatibile con questa operazione.',
            )
        if (result.outcome !== 'UPDATED' || !result.record)
            throw new CreatureTransformationRequestRepositoryError(
                'REQUEST_PERSISTENCE_FAILED',
                'La transizione persistente non ha restituito il record aggiornato.',
            )
        return result.record
    }
    ...
    async listCompletedImageRecords(input: {
        ...
        try {
            ...
            throw new CreatureTransformationRequestRepositoryError(
                'REQUEST_PERSISTENCE_FAILED',
                'Non e stato possibile leggere il catalogo delle immagini.',
                { cause: error },
            )
        }
