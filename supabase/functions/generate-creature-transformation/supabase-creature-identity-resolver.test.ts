...
describe('SupabaseCreatureIdentityResolver', () => {
    it('resolves the canonical identity without using client supplied identity fields', async () => {
        ...
        expect(result.sourceImagePath).toBe(
    ...
    it('rejects missing, foreign and unsupported creature records', async () => {
        await expect(
            new SupabaseCreatureIdentityResolver(createRepository(null)).resolve({
                profileId: 'profile-1',
                creatureId: 'missing',
            }),
        ).rejects.toMatchObject({ code: 'CREATURE_NOT_FOUND' } satisfies Partial<CreatureIdentityResolutionError>)
        await expect(
            new SupabaseCreatureIdentityResolver(
                createRepository({ id: 'creature-1', profileId: 'profile-2', baseCreatureKey: 'VERDANT_HATCHLING' }),
            ).resolve({ profileId: 'profile-1', creatureId: 'creature-1' }),
        ).rejects.toMatchObject({ code: 'CREATURE_NOT_OWNED' } satisfies Partial<CreatureIdentityResolutionError>)
        await expect(
            new SupabaseCreatureIdentityResolver(
                createRepository({ id: 'creature-1', profileId: 'profile-1', baseCreatureKey: 'UNKNOWN_CREATURE' }),
            ).resolve({ profileId: 'profile-1', creatureId: 'creature-1' }),
        ).rejects.toMatchObject({
            code: 'CREATURE_IDENTITY_NOT_SUPPORTED',
        ...
        const resolver = new SupabaseCreatureIdentityResolver(
            createRepository({ id: 'creature-1', profileId: 'profile-1', baseCreatureKey: 'VERDANT_HATCHLING' }),
            {
                VERDANT_HATCHLING: {
                    ...
                    identityFeatures: ['volto'],
                    styleDefinition: 'Stile',
                },
            },
        )

        await expect(resolver.resolve({ profileId: 'profile-1', creatureId: 'creature-1' })).rejects.toMatchObject({
            code: 'CREATURE_IDENTITY_CONFIGURATION_INVALID',
        } satisfies Partial<CreatureIdentityResolutionError>)
    })
        ...
        const history = [
            {
                ...
                evolutionTargetId: 'LIMBS_AND_FEET' as const,
                conceptName: 'Arti mediani',
                bodyPlanMutationId: 'ADD_LIMB_PAIR' as const,
            },
            ...Array.from({ length: 9 }, (_, index) => ({
                versionNumber: index + 3,
                visualTraitId: 'LOCOMOTION_ADAPTATION' as const,
                evolutionTargetId: 'TAIL' as const,
                conceptName: `Coda ${index + 3}`,
            })),
        ]
        const repository: PlayerCreatureRepository = {
            async findByCreatureId() {
                return {
                    ...
                    profileId: 'profile-1',
                    baseCreatureKey: 'VERDANT_HATCHLING',
                    currentVisualVersionId: 'version-10',
                }
            },
            async findCurrentVisualVersion() {
                return {
                    id: 'version-10',
                    creatureId: 'creature-1',
                    assetPath: 'current.png',
                    assetSha256: 'a'.repeat(64),
                    versionNumber: 10,
                    isBaseVersion: false,
                }
            },
            async listPreviousTransformations() {
                return history
            },
        }

        const result = await new SupabaseCreatureIdentityResolver(repository).resolve({
            profileId: 'profile-1',
            creatureId: 'creature-1',
        })
    ...
    it('uses the adopted corrected visual asset and its corrected facing as the next evolution source', async () => {
        ...
        const repository: PlayerCreatureRepository = {
            async findByCreatureId() {
                return {
                    ...
                    profileId: 'profile-1',
                    baseCreatureKey: 'VERDANT_HATCHLING',
                    currentVisualVersionId: 'version-2',
                }
            },
            async findCurrentVisualVersion() {
                return {
                    id: 'version-2',
                    creatureId: 'creature-1',
                    assetPath: 'candidates/profile-1/' + 'c'.repeat(64) + '.png',
                    assetSha256: 'c'.repeat(64),
                    versionNumber: 2,
                    isBaseVersion: false,
                    visualInspection: correctedInspection,
                }
            },
        }
        ...
        expect(result).toMatchObject({
            sourceImagePath: 'candidates/profile-1/' + 'c'.repeat(64) + '.png',
            sourceSha256: 'c'.repeat(64),
            visualInspection: {
                observedVisualState: { orientation: { facing: 'IMAGE_RIGHT' } },
                assetCorrection: { type: 'HORIZONTAL_MIRROR' },
            },
        })
    })
})
