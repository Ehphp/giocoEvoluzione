...
function databaseRecord(overrides: Record<string, unknown> = {}) {
    return {
        ...
        visual_trait_id: 'IMPACT_ADAPTATION',
        intensity: 2,
        prompt_template_version: null,
        concept_schema_version: null,
        source_sha256: null,
        result_sha256: null,
        result_path: null,
...
describe('SupabaseCreatureTransformationRequestRepository', () => {
    it('maps atomic reservation outcomes and passes only audit-safe metadata to the RPC', async () => {
        ...
        expect(mock.rpc).toHaveBeenCalledWith(
            ...
            expect.objectContaining({
                ...
                p_evolution_target_id: 'TORSO_AND_BACK',
                p_evolution_function: 'DEFENSE',
                p_daily_request_limit: 10,
                p_daily_budget_usd: 0,
                p_stale_request_seconds: null,
            }),
        )
    ...
    it('returns existing, daily-limit, budget and ownership outcomes without database details', async () => {
        for (const outcome of [
            ...
            const payload =
                outcome === 'EXISTING' ? { outcome, record: databaseRecord({ status: 'SUCCEEDED' }) } : { outcome }
    ...
    it('uses the transition RPC and surfaces an invalid state transition as a stable repository error', async () => {
        const success = createClient({
            ...
            record: databaseRecord({
                status: 'SUCCEEDED',
                result_path: 'profile-1/a'.padEnd(78, 'a'),
        ...
        await expect(
            repository.markSucceeded({
                ...
                profileId: 'profile-1',
                data: {
                    resultPath: 'profile-1/' + 'a'.repeat(64) + '.png',
        ...
        expect(success.rpc).toHaveBeenCalledWith(
            'transition_creature_transformation_request',
            expect.objectContaining({ p_target_status: 'SUCCEEDED' }),
        )

        const conflict = new SupabaseCreatureTransformationRequestRepository(
            createClient({ outcome: 'CONFLICT', record: databaseRecord({ status: 'SUCCEEDED' }) }).client,
        )
        await expect(conflict.markRunning({ requestId: 'request-1', profileId: 'profile-1' })).rejects.toMatchObject({
    ...
    it('reads an existing record only by the authenticated server-side profile and idempotency key', async () => {
        ...
        const repository = new SupabaseCreatureTransformationRequestRepository(mock.client)
        await expect(
            repository.getByIdempotencyKey({ profileId: 'profile-1', idempotencyKey: 'key-1' }),
    ...
    it('persists the bounded visual inspection JSON only after request success', async () => {
        const inspection: VisualInspection = {
            schemaVersion: 'visual-inspection-v1',
            inspectedAt: '2026-08-17T00:00:00.000Z',
            anomalyDetector: { status: 'COMPLETE', evidence: [] },
            visualAnomalies: [],
            stateMapper: {
                status: 'COMPLETE',
            ...
            observedVisualState: {
                ...
                shortDescription:
                    'Una creatura quadrupede dalle scaglie verdi, con una lunga coda piumata e corna arancioni.',
                orientation: { viewpoint: 'PROFILE', facing: 'IMAGE_RIGHT' },
                observedBodyPlan: 'quadruped',
                headAndEyes: 'one head',
                limbsAndLimbLikeStructures: 'four legs',
                tail: 'long feathered tail',
                hornsAntlers: 'orange horns',
                dorsalStructures: 'none',
                appendages: 'none',
                skinCovering: 'green scales',
                primaryColors: ['green', 'orange'],
                distinctiveStructures: ['feathered tail'],
                targetRegions: [],
            },
        }
        const saved = createClient({
            outcome: 'UPDATED',
            record: databaseRecord({ status: 'SUCCEEDED', visual_inspection: inspection }),
        })
        await expect(
            new SupabaseCreatureTransformationRequestRepository(saved.client).recordVisualInspection({
                requestId: 'request-1',
                profileId: 'profile-1',
            ...
            visualInspection: {
                observedVisualState: { shortDescription: inspection.observedVisualState!.shortDescription },
            },
        })
        expect(saved.rpc).toHaveBeenCalledWith(
            'record_creature_transformation_visual_inspection',
            expect.objectContaining({
                p_request_id: 'request-1',
                p_profile_id: 'profile-1',
                p_visual_inspection: inspection,
            }),
        )
    })

    it('persists a queue submission and uses the server-only finalization claim RPC', async () => {
        const submission = createClient({
            outcome: 'UPDATED',
            record: databaseRecord({
                status: 'RUNNING',
                provider: 'fal.ai',
                model: 'fal-ai/flux-2-klein/9b/edit',
                provider_request_id: 'fal-job-2',
                fal_workflow: {
                    version: 1,
                    kind: 'FLUX',
                    source: { kind: 'CANONICAL', path: 'verdant-hatchling-v1.png', isBaseVersion: true },
                },
            }),
        })
        ...
        await expect(
            repository.updateRunningFalSubmission({
                requestId: 'request-1',
                profileId: 'profile-1',
                data: {
                    provider: 'fal.ai',
                    model: 'fal-ai/flux-2-klein/9b/edit',
                    providerRequestId: 'fal-job-2',
        ...
        expect(submission.rpc).toHaveBeenCalledWith(
            'update_running_fal_submission',
            expect.objectContaining({
                p_provider_request_id: 'fal-job-2',
                p_fal_workflow: expect.objectContaining({ kind: 'FLUX' }),
                p_expected_provider_request_id: null,
            }),
        )

        const claim = createClient({
            outcome: 'CLAIMED',
            record: databaseRecord({
                status: 'RUNNING',
                provider_request_id: 'fal-job-2',
                fal_finalization_request_id: 'fal-job-2',
            }),
        })
        await expect(
            new SupabaseCreatureTransformationRequestRepository(claim.client).claimFalFinalization({
                providerRequestId: 'fal-job-2',
            }),
        ).resolves.toMatchObject({ outcome: 'CLAIMED', record: { falFinalizationRequestId: 'fal-job-2' } })
        expect(claim.rpc).toHaveBeenCalledWith('claim_fal_transformation_finalization', {
            p_provider_request_id: 'fal-job-2',
        })
    })
})
