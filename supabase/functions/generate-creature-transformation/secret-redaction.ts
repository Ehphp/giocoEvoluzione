const URL_SECRET_PARAMETER =
    /([?&](?:access_token|api[_-]?key|token|key|secret|password|fal_callback_token)=)[^&#\s]+/gi
const LABELLED_SECRET =
    /(\b(?:authorization|api[_-]?key|token|secret|password|client_secret|private_key)\b["']?\s*[:=]\s*)(?:(?:Bearer|Key)\s+)?(?:"[^"]*"|'[^']*'|[^\s,;}\]]+)/gi
const PROVIDER_SECRET =
    /\b(?:sk-[A-Za-z0-9_-]{12,}|sb_secret_[A-Za-z0-9_-]{12,}|AIza[0-9A-Za-z_-]{20,}|gh[pousr]_[A-Za-z0-9_]{16,}|github_pat_[A-Za-z0-9_]{16,}|nfp_[A-Za-z0-9]{16,}|[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}:[0-9a-f]{32,})\b/gi
