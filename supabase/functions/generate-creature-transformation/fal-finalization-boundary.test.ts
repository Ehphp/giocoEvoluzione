...
describe('Fal webhook and finalization boundary', () => {
    ...
    it('inspects and corrects the Seedream raw before persistence and background removal, without entering retry flow', () => {
        expect(seedreamFinalizer.indexOf('inspectSeedreamVisual')).toBeLessThan(
            seedreamFinalizer.indexOf('flipImageHorizontallyToPng'),
        )
        ...
        expect(seedreamFinalizer.indexOf('recordVisualInspection')).toBeLessThan(
            seedreamFinalizer.indexOf('markBackgroundRemovalPending'),
        )
        ...
        expect(seedreamFinalizer.slice(seedreamFinalizer.indexOf('inspectSeedreamVisual'))).not.toContain(
            'retryCroppedSeedream',
        )
    })
    ...
    it('rejects a Vision-verified center-facing Seedream image before it can reach raw storage or background removal', () => {
        ...
        expect(seedreamFinalizer.indexOf('seedream_orientation_rejected')).toBeLessThan(
            seedreamFinalizer.indexOf('saveRawResult'),
        )
        expect(seedreamFinalizer.indexOf('seedream_orientation_rejected')).toBeLessThan(
            seedreamFinalizer.indexOf('markBackgroundRemovalPending'),
