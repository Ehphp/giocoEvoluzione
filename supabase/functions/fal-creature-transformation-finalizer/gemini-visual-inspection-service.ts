...
export function readGeminiVisualInspectionConfiguration(
    readEnvironment: (name: string) => string | undefined,
): GeminiVisualInspectionConfiguration {
...
const EVIDENCE_SCHEMA = responseSchema(
    'OBJECT',
    {
        ...
        imageRegion: responseSchema('STRING', {
            enum: [
                'IMAGE_LEFT',
                'IMAGE_RIGHT',
                'UPPER_IMAGE_LEFT',
...
const OBSERVED_STATE_SCHEMA = responseSchema(
    'OBJECT',
    {
        shortDescription: responseSchema('STRING'),
        orientation: responseSchema(
            'OBJECT',
            {
        ...
        targetRegions: responseSchema('ARRAY', {
...
const DETECTOR_SCHEMA = responseSchema(
    'OBJECT',
    {
...
const MAPPER_SCHEMA = responseSchema(
    'OBJECT',
    {
        observedVisualState: OBSERVED_STATE_SCHEMA,
        evidenceAssessments: responseSchema('ARRAY', {
            items: responseSchema(
                'OBJECT',
                {
                    evidenceIndex: responseSchema('INTEGER'),
                    disposition: responseSchema('STRING', {
                        enum: ['CONFIRMED', 'POSSIBLE', 'REJECTED_WITH_STRONG_CONTRARY_EVIDENCE'],
                    }),
...
function detectorPrompt(input: { bodyPlan: CreatureBodyPlan; expectedOrientation?: string | null }): string {
    ...
    return [
        ...
        input.expectedOrientation
            ? `Expected source presentation reference: ${input.expectedOrientation}.`
            : 'No prior presentation reference is available; report orientation only when a defect is visually supportable.',
...
function mapperPrompt(evidence: readonly Vision1DiagnosticEvidence[]): string {
    const upstream = evidence.length ? JSON.stringify(evidence) : '[]'
    return [
        ...
        'Write shortDescription in Italian: exactly one natural, lightly evocative character-card sentence, ideally 15-25 words, using 2-4 distinctive traits visibly present now. Make it a compact visual signature: choose the traits that distinguish this individual from a generic creature with the same body plan. Prefer a specific silhouette or anatomy, the shape/placement of a notable appendage, a visible covering or texture, and a precise colour-pattern pairing. Give each chosen trait a concrete qualifier when visible (for example material plus colour, or shape plus placement). Do not spend words on generic praise or common traits: never say "occhi espressivi", "aspetto elegante", "creatura graziosa" or "colori vivaci". Mention eyes only for a visibly distinctive detail such as an unusual shape, colour, pupil or placement. It may mention anatomy, body shape, colours, covering, tail, horns, dorsal structures or appendages. Do not mention camera, framing, facing, Vision, anomalies, errors, generations, pipelines or evolutionary history. Do not invent hidden traits. Never use report formulas such as "The image shows...".',
...
export class GeminiVisualInspectionService {
    ...
    constructor(
        private readonly configuration: GeminiVisualInspectionConfiguration,
    ...
    private async deleteFile(file: GeminiFile): Promise<void> {
        const response = await this.fetchImplementation(
            ...
            {
                method: 'DELETE',
                headers: { 'x-goog-api-key': this.configuration.apiKey! },
            },
        )
    ...
    private async detect(
        ...
        const payload = await this.generateJson(
            file,
            detectorPrompt({ bodyPlan, expectedOrientation }),
            DETECTOR_SCHEMA,
            this.configuration.detectorTimeoutMs,
        )
        const root = record(payload)
    ...
    private async map(file: GeminiFile, evidence: readonly Vision1DiagnosticEvidence[]): Promise<MapperResult> {
        const payload = await this.generateJson(
            file,
            mapperPrompt(evidence),
            MAPPER_SCHEMA,
            this.configuration.mapperTimeoutMs,
        )
        const root = record(payload)
        const observedVisualState = parseObservedVisualState(root?.observedVisualState)
        const assessments = Array.isArray(root?.evidenceAssessments)
            ? root.evidenceAssessments.map(parseMapperEvidenceAssessment)
            : null
        const structuralConcerns = Array.isArray(root?.structuralConcerns)
            ? root.structuralConcerns.map(parseVision1DiagnosticEvidence)
            : null
        const validAssessments =
            assessments && assessments.every((entry): entry is MapperEvidenceAssessment => entry !== null)
        const validConcerns =
            structuralConcerns &&
            structuralConcerns.every((entry): entry is Vision1DiagnosticEvidence => entry !== null)
        const hasEveryAssessment =
            evidence.length === 0 ||
            (validAssessments &&
                new Set(assessments.map((assessment) => assessment.evidenceIndex)).size === evidence.length &&
                assessments.every((assessment) => assessment.evidenceIndex < evidence.length))
        return observedVisualState && validAssessments && validConcerns && hasEveryAssessment
            ? Object.freeze({
                  status: 'COMPLETE',
                  usedVision1Evidence: evidence.length > 0,
