...
import {
    ...
    type GeminiVisualInspectionConfiguration,
} from './gemini-visual-inspection-service.ts'

const configuration: GeminiVisualInspectionConfiguration = Object.freeze({
    enabled: true,
    apiKey: 'test-key',
...
const detectorEvidence = {
    ...
    confidence: 0.93,
    description: 'Additional limb-like structure detected.',
}
const observed = {
    shortDescription:
        "Una minuta creatura quadrupede dalle scaglie verde giada, con un quinto arto laterale, coda piumata e corna ricurve d'ambra.",
    orientation: { viewpoint: 'PROFILE', facing: 'IMAGE_RIGHT' },
    observedBodyPlan: 'quadruped',
    headAndEyes: 'one head',
    limbsAndLimbLikeStructures: 'four limbs plus a possible extra limb',
    tail: 'one tail',
    hornsAntlers: 'none',
    dorsalStructures: 'none',
    appendages: 'possible extra limb',
    skinCovering: 'scales',
    primaryColors: ['green'],
...
function fetchFor(input: {
    ...
    return vi.fn(async (url: string | URL | Request, init?: RequestInit) => {
        ...
        if (href === 'https://upload.example.test/session')
            return new Response(
                JSON.stringify({ file: { name: 'files/one', uri: 'gemini://files/one', mimeType: 'image/png' } }),
                { status: 200 },
            )
...
describe('GeminiVisualInspectionService', () => {
    it('uses configurable Vision timeouts with conservative defaults', () => {
        expect(
            readGeminiVisualInspectionConfiguration((name) => ({ GEMINI_API_KEY: 'key' })[name]).detectorTimeoutMs,
        ).toBe(4_000)
        expect(
            readGeminiVisualInspectionConfiguration(
                (name) =>
                    ({
                        GEMINI_API_KEY: 'key',
                        CREATURE_VISION_1_TIMEOUT_MS: '5000',
                        CREATURE_VISION_2_TIMEOUT_MS: '7000',
                    })[name],
            ).mapperTimeoutMs,
        ).toBe(7_000)
    })
    ...
    it('runs Vision 1 before Vision 2 and injects its structured diagnostic evidence into the mapper prompt', async () => {
        ...
        const service = new GeminiVisualInspectionService(
            ...
            fetchFor({
                ...
                mapper: {
                    ...
                    structuralConcerns: [detectorEvidence],
                },
                onMapper: (body) => {
                    mapperBody = body
                },
        ...
        const mapperPrompt = String(
            (mapperBody!.contents as Array<{ parts: Array<{ text?: string }> }>)[0].parts[0].text,
        )
    ...
    it('persists mapper POSSIBLE and strong-contrary-evidence assessments without letting them alter canonical anatomy', async () => {
            ...
            fetchFor({
                ...
                mapper: {
                    ...
                    evidenceAssessments: [
                        {
                            evidenceIndex: 0,
                            disposition: 'REJECTED_WITH_STRONG_CONTRARY_EVIDENCE',
                            verificationNote: 'clear contrary geometry',
                        },
                    ],
                    structuralConcerns: [],
                },
        ...
        expect(result.stateMapper.evidenceAssessments).toEqual([
            expect.objectContaining({ disposition: 'REJECTED_WITH_STRONG_CONTRARY_EVIDENCE' }),
        ])
    ...
    it('fails open when both Vision calls are unavailable', async () => {
        const service = new GeminiVisualInspectionService(
            configuration,
            fetchFor({ detector: { unexpected: true }, mapper: { unexpected: true } }),
        )
        await expect(
            service.inspect({
                image: new Uint8Array([1]),
            ...
            anomalyDetector: { status: 'UNAVAILABLE' },
            stateMapper: { status: 'UNAVAILABLE' },
            visualAnomalies: [],
        })
    })
    ...
    it('treats a Vision 1 timeout as unavailable and still runs Vision 2', async () => {
        ...
        const fetch = vi.fn(async (url: string | URL | Request, init?: RequestInit) => {
            ...
            if (
                href.includes(':generateContent') &&
                String(init?.body).includes('specialized visual anomaly detector')
            ) {
            ...
            if (href.includes(':generateContent'))
                return generated({ observedVisualState: observed, evidenceAssessments: [], structuralConcerns: [] })
            return new Response('', { status: 200 })
        }) as unknown as typeof fetch
        const result = await new GeminiVisualInspectionService(timedConfiguration, fetch).inspect({
            image: new Uint8Array([1]),
            mimeType: 'image/png',
            bodyPlan: BODY_PLANS.QUADRUPED,
            generation: 2,
