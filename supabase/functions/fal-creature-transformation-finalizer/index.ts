...
} from '../../../shared/creature-transformations/flux-evolution/body-plan-registry.ts'
import {
    SupabaseCreatureIdentityResolver,
    type PlayerCreatureRepository,
} from '../generate-creature-transformation/supabase-creature-identity-resolver.ts'
import {
    SupabaseCreatureTransformationStorageAdapter,
    type CreatureTransformationStorageClient,
} from '../generate-creature-transformation/supabase-creature-transformation-storage.ts'
import {
...
} from '../generate-creature-transformation/creature-transformation-request-repository.ts'
import {
    SupabaseCreatureVisualProgressionRepository,
    type CreatureVisualProgressionRepositoryClient,
} from '../generate-creature-transformation/creature-visual-progression-repository.ts'
import {
...
import {
    FLUX_MAX_CROP_RETRIES,
    FLUX_SUBJECT_MARGIN_RATIO,
    FluxImageGenerationServiceError,
} from '../generate-creature-transformation/flux-image-generation-service.ts'
import {
    composeSeedreamQueuePrompt,
    fluxMicroConceptFromSnapshot,
} from '../generate-creature-transformation/fal-queue-submission-service.ts'
        ...
        async findCurrentVisualVersion({ creatureId, versionId }) {
            ...
            return data
                ? {
                      ...
                      isBaseVersion:
                          !/^(?:[A-Za-z0-9-]{1,128}\/[a-f0-9]{64}|experiments\/raw\/[A-Za-z0-9-]{1,128}\/[a-f0-9]{64}|candidates\/[A-Za-z0-9-]{1,128}\/[a-f0-9]{64}|cleanup\/[a-f0-9]{64})\.png$/.test(
                              String(data.asset_path),
                          ),
            ...
            : (() => {
                  ...
                  if (!supabaseUrl || !/^https:\/\//.test(supabaseUrl)) {
                      throw new FluxImageGenerationServiceError(
                          ...
                          'L URL del webhook Fal non e configurato.',
                      )
                  }
...
function seedreamImageDimensions(input: { bytes: Uint8Array; mimeType: 'image/png' | 'image/jpeg' }): {
    width: number
    height: number
} {
    ...
    if (
        !dimensions ||
        input.bytes.length > 30 * 1024 * 1024 ||
        ...
        throw new FluxImageGenerationServiceError(
            ...
            'L output Seedream non ha dimensioni o peso accettabili.',
        )
    }
    return dimensions
}

function seedreamProductionDimensions(input: {
    bytes: Uint8Array
    ...
    if (dimensions.width !== input.expected.width || dimensions.height !== input.expected.height) {
        throw new FluxImageGenerationServiceError(
            'FLUX_RESULT_IMAGE_INVALID',
            'L output Seedream non rispetta il formato portrait persistito per questa richiesta.',
        )
    }
    return dimensions
}
...
async function restoreTrack(
    ...
    try {
        await visualRepository.completeGeneration({
            profileId: record.profileId,
            trackId: record.visualProgressTrackId,
            requestId: record.id,
            finalAsset: false,
        })
    } catch (error) {
        console.error('fal.finalizer.track_restore_failed', {
...
async function failRequest(
    ...
    const code =
        error instanceof FluxImageGenerationServiceError
            ? error.code
            : error && typeof error === 'object' && typeof (error as { code?: unknown }).code === 'string'
              ? (error as { code: string }).code
              : 'FAL_FINALIZATION_FAILED'
    const message =
        error instanceof FluxImageGenerationServiceError
    ...
    try {
        ...
        /* terminal/duplicate requests are already safe */
    }
...
async function retryCroppedSeedream(input: {
    ...
    if (
        !workflow ||
        workflow.kind !== 'SEEDREAM_PRODUCTION' ||
        !concept ||
        !snapshot ||
        !capability ||
        !input.record.providerRequestId ||
        !input.record.promptTemplateVersion ||
        (capability === 'BODY_PLAN_MUTATION' && !bodyPlanMutationId)
    ) {
        throw new FluxImageGenerationServiceError(
            'FLUX_RESULT_IMAGE_INVALID',
            'I metadati della submission Seedream non sono disponibili per il retry di framing.',
        )
    }
    ...
    const bodyPlan =
        source.bodyPlan ??
        resolveCanonicalBodyPlan({
            baseCreatureKey: source.identity.baseCreatureKey,
            adoptedBodyPlanMutationIds: source.adoptedBodyPlanMutationIds,
        })
    if (!bodyPlan || !input.record.evolutionTargetId)
        throw new FluxImageGenerationServiceError(
            'FLUX_BODY_PLAN_UNSUPPORTED',
            'La topologia della richiesta Seedream non e disponibile per il retry.',
        )
    ...
    const sourceUrl = (
        await input.storage.createVisualVersionSignedUrl({
            assetPath: workflow.source.path,
            isBaseVersion: workflow.source.isBaseVersion,
            expiresInSeconds: input.sourceUrlTtlSeconds,
        })
    ).signedUrl
    const prompt = await composeSeedreamQueuePrompt({
        identity: source.identity,
        plan,
        concept,
        framingAttempt: input.record.attemptCount,
        repairBrief: visualRepairBrief(source.visualInspection),
    })
    const submission = await input.provider.submitSeedreamEvolution({
        prompt: prompt.prompt,
        sourceUrl,
        imageSize: workflow.parameters.imageSize,
...
async function inspectSeedreamVisual(input: {
    ...
    try {
        ...
        const inspection = await new GeminiVisualInspectionService(
            readGeminiVisualInspectionConfiguration((name) => Deno.env.get(name)),
        ).inspect({
            image: input.image,
...
async function finalizeSeedreamProduction(input: {
    ...
    if (
        ...
        throw new FluxImageGenerationServiceError(
            'FLUX_RESULT_IMAGE_INVALID',
            'Metadati Seedream incompleti per la finalizzazione.',
        )
    }
    const downloaded = await input.provider.downloadQueuedImage(input.image)
    console.info('fal.finalizer.seedream_production_downloaded', {
        providerRequestId: input.record.providerRequestId,
    ...
    if (downloaded.mimeType === 'image/png') {
        ...
        const validation = await new ImageValidator().validate({
            ...
            mimeType: 'image/png',
            renderSpecification: workflow.parameters.imageSize as Parameters<
                ImageValidator['validate']
            >[0]['renderSpecification'],
        ...
        if (!validation.valid) {
            const cropFailure = validation.problems.some(
                (problem) =>
                    problem.code === 'FLUX_SUBJECT_CROPPED' || problem.code === 'PNG_FOREGROUND_DETECTION_FAILED',
            )
            ...
            throw new FluxImageGenerationServiceError(
                cropped ? 'FLUX_SUBJECT_CROPPED' : 'FLUX_RESULT_IMAGE_INVALID',
                cropped
                    ? 'Il soggetto Seedream resta troppo vicino al bordo dopo i retry di framing.'
                    : 'Il PNG raw Seedream non ha superato i controlli tecnici.',
                validation.problems,
            )
        }
    ...
    if (visualInspection && shouldRejectSeedreamCenterFacing(visualInspection.inspection)) {
        ...
        throw new FluxImageGenerationServiceError(
            ...
            'Output Seedream scartato: Vision ha rilevato una posa frontale/centrale.',
        )
    }
    ...
    if (visualInspection) {
        ...
        if (mirrorDecision.action === 'FLIP') {
            const mirrored = await flipImageHorizontallyToPng({
                bytes: downloaded.bytes,
                mimeType: downloaded.mimeType,
            })
    ...
    let completed = await input.repository.markSucceeded({
        ...
        data: {
            ...
            sourceSha256: input.record.sourceSha256,
            resultSha256,
            resultPath: await input.storage.createRawResultObjectPath(
    ...
    if (persistedInspection) {
        try {
            completed = await input.repository.recordVisualInspection({
                requestId: completed.id,
                profileId: completed.profileId,
                visualInspection: persistedInspection,
            })
        } catch (error) {
    ...
    if (completed.visualProgressTrackId)
        await input.visualRepository.markBackgroundRemovalPending({
            profileId: completed.profileId,
            trackId: completed.visualProgressTrackId,
            requestId: completed.id,
        })
}
Deno.serve(async (request) => {
    ...
    const repository = new SupabaseCreatureTransformationRequestRepository(
    ...
    if (!workflow || !record.model) {
        await failRequest(
            ...
            visualRepository,
            record,
            new FluxImageGenerationServiceError(
                'FLUX_RESULT_IMAGE_INVALID',
                'Il workflow Fal persistito non e valido.',
            ),
        )
        return json(202)
    }
    ...
    if (!providerPolicy.apiKey) {
        await failRequest(
            ...
            new FluxImageGenerationServiceError(
                'FAL_FLUX_NOT_CONFIGURED',
                'La chiave del provider persistito non e configurata.',
            ),
        )
        return json(202)
    }
