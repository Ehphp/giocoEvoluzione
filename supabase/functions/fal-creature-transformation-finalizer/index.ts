...
import { composeFluxQueuePrompt, fluxMicroConceptFromSnapshot } from '../generate-creature-transformation/fal-queue-submission-service.ts'
...
Deno.serve(async (request) => {
    ...
    if (!policy.flux.apiKey) return json(500)
    ...
    try {
        ...
        if (workflow.kind === 'FLUX') {
            ...
            const microConceptGenerator = workflow.conceptSource === 'dynamic'
                ...
                : null
            await finalizeSeedream({ record, image, provider, repository, storage, resolver, microConceptGenerator, sourceUrlTtlSeconds: policy.flux.submissionSourceUrlTtlSeconds })
        }
    } catch (error) {
