-- Seedream production may return a JPEG raw result. It is private intermediate
-- material only: browser post-processing must still promote an exact transparent
-- 1024x1536 PNG candidate before the visual can become adoptable.
...
begin
  ...
  if v_request.operation <> 'GENERATE_UNLOCKED_TRANSFORMATION'
    ...
    or v_request.result_path !~ ('^experiments/raw/' || p_profile_id::text || '/[a-f0-9]{64}\.(png|jpg)$')
    or v_request.result_sha256 is null
    or v_request.result_mime_type not in ('image/png', 'image/jpeg')
    or not (
