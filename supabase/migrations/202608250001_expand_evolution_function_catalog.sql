-- The evolution-function column remains text. The reservation RPC owns the accepted catalogue,
-- so extend that validator without changing quota, idempotency or persistence semantics.
...
begin
  ...
  if p_evolution_function is not null and p_evolution_function not in ('BALANCE','PROPULSION','GRIP','DEFENSE','PERCEPTION','THERMOREGULATION','ENERGY_STORAGE','IMPACT_ABSORPTION','AQUATIC_ADAPTATION','CAMOUFLAGE','MANEUVERABILITY','ENDURANCE','ACCELERATION','IMPACT_RESISTANCE','OXYGEN_EFFICIENCY') then raise exception 'invalid evolution function'; end if;
