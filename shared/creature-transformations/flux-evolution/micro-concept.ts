...
import {
    isBodyPlanMutationId,
    isEvolutionCapability,
...
export type FluxEvolutionSnapshot = FluxMicroConcept &
    Readonly<{
...
export function parseFluxMicroConcept(value: unknown): FluxMicroConcept | null {
    if (
        !record(value) ||
        Object.keys(value).some((key) => !['conceptName', 'mutationIdea', 'visualDetails', 'avoid'].includes(key))
    )
        return null
    ...
    if (
        ...
        (value.avoid !== undefined && avoid === null)
    )
        return null
    return Object.freeze({
        conceptName,
        mutationIdea,
        visualDetails: Object.freeze(visualDetails),
        ...(avoid?.length ? { avoid: Object.freeze(avoid) } : {}),
    })
}

export function createFluxEvolutionSnapshot(
    input: FluxMicroConcept & {
        evolutionTargetId: EvolutionTargetId
        evolutionFunction: EvolutionFunctionId
        capability: EvolutionCapability
        bodyPlanMutationId?: BodyPlanMutationId
...
export function isFluxEvolutionSnapshot(value: unknown): value is FluxEvolutionSnapshot {
    ...
    if (value.schemaVersion !== FLUX_MICRO_CONCEPT_SCHEMA_VERSION && value.schemaVersion !== LEGACY_SCHEMA_VERSION)
        return false
    ...
    if (
        ...
        (typeof providerSeed !== 'number' || !Number.isInteger(providerSeed) || providerSeed < 0)
    )
        return false
    return (
        parseFluxMicroConcept({
            ...
            ...(value.avoid === undefined ? {} : { avoid: value.avoid }),
        }) !== null
    )
}
