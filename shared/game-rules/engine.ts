...
function cloneAdaptations(adaptations: AdaptationCollection): AdaptationCollection {
    return Object.fromEntries(
        Object.entries(adaptations).map(([adaptation, state]) => [adaptation, { ...state }]),
...
export function isAdaptationUsable(adaptations: AdaptationCollection, adaptation: AdaptationId): boolean {
    return !adaptations[adaptation].exhausted
}
export function isAdaptationEvolvable(adaptations: AdaptationCollection, adaptation: AdaptationId): boolean {
...
export function isCombatMutationEquipped(loadout: readonly CombatMutationId[], mutation: CombatMutationId): boolean {
    return loadout.includes(mutation)
}
export function getCombatMutationUseBonus(state: CombatMutationState, loadout: CombatMutationLoadout): number {
    return state.adaptiveCoreStatus === 'ARMED' && isCombatMutationEquipped(loadout, 'ADAPTIVE_CORE') ? 1 : 0
}
/** Presentation and bot previews use this rather than reproducing mutation conditions. */
export function getCombatMutationUsePreview(
    state: CombatMutationState,
    adaptation: AdaptationId,
    loadout: CombatMutationLoadout,
) {
    return {
        mutationBonus: getCombatMutationUseBonus(state, loadout),
        elasticLimbsWillPreserveAgility:
            adaptation === 'AGILITY' && !state.elasticLimbsUsed && isCombatMutationEquipped(loadout, 'ELASTIC_LIMBS'),
        armoredMemoryWillPreserveArmor:
            adaptation === 'ARMOR' && !state.armoredMemoryUsed && isCombatMutationEquipped(loadout, 'ARMORED_MEMORY'),
    }
}
export function getCombatMutationEvolvePreview(
    state: CombatMutationState,
    ...
    return {
        mutationBonus:
            adaptations[adaptation].exhausted &&
            !state.recoverySurgeUsed &&
            isCombatMutationEquipped(loadout, 'RECOVERY_SURGE')
                ? 1
                : 0,
        adaptiveCoreWillArm:
            state.adaptiveCoreStatus === 'DORMANT' && isCombatMutationEquipped(loadout, 'ADAPTIVE_CORE'),
    }
}
/** Shared transition preview used by both resolution and bot evaluation. */
export function getCombatMutationStateAfterEvolve(
    state: CombatMutationState,
    loadout: CombatMutationLoadout,
): CombatMutationState {
    const nextState = cloneCombatMutationState(state)
    if (nextState.adaptiveCoreStatus === 'DORMANT' && isCombatMutationEquipped(loadout, 'ADAPTIVE_CORE'))
        nextState.adaptiveCoreStatus = 'ARMED'
    return nextState
}
export function getAdaptationRoundValue(
    ...
    return getValidatedAdaptationUseBreakdown(
        roundEvent,
        adaptations,
        adaptation,
        0,
        getCombatMutationUseBonus(combatMutationState, combatMutationLoadout),
    ).total
}
export function getEvolutionRoundValue(
    roundEvent: EnvironmentalCrisisDefinition,
    adaptations: AdaptationCollection,
    adaptation: AdaptationId,
    combatMutationState: CombatMutationState,
    ...
    return getValidatedActionBreakdown(
        roundEvent,
        adaptations,
        adaptation,
        'EVOLVE',
        0,
        getCombatMutationEvolvePreview(combatMutationState, adaptations, adaptation, combatMutationLoadout)
            .mutationBonus,
    ).total
}
...
function resolvePlayerAction(
    ...
    if (action.actionType === 'EVOLVE') {
        if (!isAdaptationEvolvable(adaptations, action.trait))
            throw new Error(
                `Adaptation ${action.trait} is already available at the maximum level; EVOLVE would produce no transition.`,
            )
        const mutationPreview = getCombatMutationEvolvePreview(
            combatMutationState,
            adaptations,
            action.trait,
            combatMutationLoadout,
        ...
        return {
            ...
            breakdown,
            traits: nextAdaptations,
            combatMutationState: getCombatMutationStateAfterEvolve(nextCombatMutationState, combatMutationLoadout),
            mutationEffects,
    ...
    if (!isAdaptationUsable(adaptations, action.trait))
        throw new Error(`Adaptation ${action.trait} is exhausted and cannot be used.`)
...
function validateLinksAtRoundStart(
    links: readonly SymbiosisLink[],
    player1Id: string,
    player2Id: string,
    roundNumber: number,
): void {
    if (links.length > 2 || new Set(links.map((link) => link.ownerPlayerId)).size !== links.length)
        throw new Error('Invalid SYMBIOSIS link ownership.')
    for (const link of links) {
        const expectedTarget =
            link.ownerPlayerId === player1Id ? player2Id : link.ownerPlayerId === player2Id ? player1Id : null
        if (!expectedTarget || link.targetPlayerId !== expectedTarget || link.activatedRound >= roundNumber)
            throw new Error('Invalid SYMBIOSIS link for this round.')
    }
}
...
export function resolveRound(input: ResolveRoundInput): RoundResolution {
    ...
    const player1 = resolvePlayerAction(
        ...
        input.player2Id,
        input.player1Traits,
        input.player1CombatMutationState,
        input.player1CombatMutationLoadout,
        input.player1Action,
        input.player2Action,
        linksAtStart,
    ...
    const player2 = resolvePlayerAction(
        ...
        input.player1Id,
        input.player2Traits,
        input.player2CombatMutationState,
        input.player2CombatMutationLoadout,
        input.player2Action,
        input.player1Action,
        linksAtStart,
        fineDelMondoActivationsAtStart,
    ...
    for (const target of propagationTargets) {
        const traits =
            target.targetPlayerId === input.player1Id
                ? player1.traits
                : target.targetPlayerId === input.player2Id
                  ? player2.traits
                  : null
    ...
    const activatedLinks = [player1.activationLink, player2.activationLink].filter((link): link is SymbiosisLink =>
        Boolean(link),
    )
    ...
    return {
        ...
        player1: {
            ...
            combatMutationState: player1.combatMutationState,
            mutationEffects: player1.mutationEffects,
        },
        player2: {
            ...input.player2Action,
            ...
            combatMutationState: player2.combatMutationState,
            mutationEffects: player2.mutationEffects,
        },
        winnerId: player1Won ? input.player1Id : player2Won ? input.player2Id : null,
        awardedPoints,
