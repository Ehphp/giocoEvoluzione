...
const action = (playerId: string, trait: (typeof ADAPTATION_IDS)[number], actionType: 'USE' | 'EVOLVE') => ({
    playerId,
    trait,
    actionType,
})
const round = (overrides: Partial<Parameters<typeof resolveRound>[0]> = {}) =>
    resolveRound({
        roundNumber: 1,
        roundEvent: crisis,
        player1Id: 'p1',
...
describe('exhaustion best-of-seven rules', () => {
    it('starts every adaptation available at level zero and normalizes JSONB safely', () => {
        ...
        expect(
            normalizeAdaptationCollection({
                FEROCITY: { level: 99, exhausted: true },
                ARMOR: { level: -3, exhausted: false },
            }),
        ).toMatchObject({ FEROCITY: { level: 2, exhausted: true }, ARMOR: { level: 0, exhausted: false } })
        expect(
            normalizeAdaptationCollection(JSON.parse(JSON.stringify({ FEROCITY: { level: 1, exhausted: true } }))),
        ).toMatchObject({ FEROCITY: { level: 1, exhausted: true } })
    })
    ...
    it('rejects USE on an exhausted adaptation and never recovers it automatically', () => {
        ...
        adaptations.FEROCITY.exhausted = true
        const otherRound = round({
            player1Traits: adaptations,
    ...
    it('EVOLVE always yields one, increments below max and recovers available or exhausted adaptations', () => {
        ...
        for (const exhausted of [false, true]) {
            const adaptations = createInitialAdaptations()
            adaptations.FEROCITY.exhausted = exhausted
            const result = round({
                player1Traits: adaptations,
            ...
            expect(result.player1.traits.FEROCITY).toEqual({ level: 1, exhausted: false })
        }
    })
    it('EVOLVE recovers an exhausted max-level adaptation and rejects a no-op', () => {
        const exhausted = createInitialAdaptations()
        exhausted.FEROCITY.level = 2
        exhausted.FEROCITY.exhausted = true
        expect(
            round({
                player1Traits: exhausted,
                player1Action: action('p1', 'FEROCITY', 'EVOLVE'),
                player2Action: action('p2', 'ARMOR', 'EVOLVE'),
            }).player1.traits.FEROCITY,
        ).toEqual({ level: 2, exhausted: false })
        exhausted.FEROCITY.exhausted = false
        expect(() => round({ player1Traits: exhausted, player1Action: action('p1', 'FEROCITY', 'EVOLVE') })).toThrow(
            'no transition',
        )
    })
    ...
    it('is idempotent at the resolution boundary and preserves exhausted state in persisted mapping', () => {
        ...
        const first = buildPersistedRoundResolution(params)
        const replay = buildPersistedRoundResolution(params)
        expect(replay).toEqual(first)
        expect(first.resolution_data.player1TraitsAfter.FEROCITY.exhausted).toBe(true)
    })
    it('limits every event affinity to 0, 1 or 2', () => {
        for (const event of ROUND_EVENT_DEFINITIONS)
            for (const modifier of Object.values(event.modifiers)) expect([0, 1, 2]).toContain(modifier)
    })
    it('ends immediately at four wins and preserves round-seven stored-value tiebreaks', () => {
        const result = buildPersistedRoundResolution({
            ...
            player1CombatMutationLoadout: BOT_COMBAT_MUTATION_LOADOUT,
            player2CombatMutationLoadout: BOT_COMBAT_MUTATION_LOADOUT,
            player1Action: action('p1', 'SENSES', 'USE'),
        ...
        expect(
            resolveMatchOutcome({
                ...
                storedRoundValues: [{ player1Value: 2, player2Value: 1 }],
            }),
        ).toMatchObject({ finished: true, winnerId: 'p1', reason: 'ROUND_VALUE_TIEBREAK' })
    })
})
