...
export type PersistedRoundResolutionData = {
    ...
    player1TraitsAfter: GeneCollection
    player2TraitsAfter: GeneCollection
...
export function buildPersistedRoundResolution(params: {
    roundNumber: number; roundEvent: RoundEventDefinition; player1Id: string; player2Id: string
    player1Score: number; player2Score: number; player1Traits: GeneCollection; player2Traits: GeneCollection
    player1Action: PlayerRoundAction; player2Action: PlayerRoundAction; startedAt: string | null; now?: () => string
}) {
    ...
    const isFinalRound = params.roundNumber === TOTAL_ROUNDS
    const finishedAt = isFinalRound ? (params.now ?? (() => new Date().toISOString()))() : null
    const winnerIdAfter = !isFinalRound || player1ScoreAfter === player2ScoreAfter ? null : player1ScoreAfter > player2ScoreAfter ? params.player1Id : params.player2Id
    return {
        ...
        resolution_data: {
            ...
            player1TraitsAfter: resolution.player1.traits, player2TraitsAfter: resolution.player2.traits,
            player1ScoreAfter, player2ScoreAfter, statusAfter: isFinalRound ? 'FINISHED' : 'REVEALING', winnerIdAfter, finishedAt,
            durationMs: params.startedAt && finishedAt ? new Date(finishedAt).getTime() - new Date(params.startedAt).getTime() : null,
