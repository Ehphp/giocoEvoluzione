...
export function getValidatedAdaptationUseBreakdown(roundEvent: EnvironmentalCrisisDefinition, adaptations: AdaptationCollection, adaptation: AdaptationId, matchupBonus = 0, mutationBonus = 0): RoundValueBreakdown {
    ...
    return { actionType: 'USE', baseContribution: BASE_USE_VALUE, levelContribution, eventModifier, matchupBonus, mutationBonus, originalLevel: state.level, effectiveLevel, total: BASE_USE_VALUE + levelContribution + eventModifier + matchupBonus + mutationBonus, appliedEventEffects }
}
