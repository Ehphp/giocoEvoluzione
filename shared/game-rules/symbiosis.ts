...
export function canonicalSymbiosisPairKey(
    link: Pick<SymbiosisLink, 'ownerPlayerId' | 'sourceTrait' | 'targetPlayerId' | 'targetTrait'>,
): string {
    return [`${link.ownerPlayerId}:${link.sourceTrait}`, `${link.targetPlayerId}:${link.targetTrait}`].sort().join('|')
}

function endpointKey(playerId: string, trait: AdaptationId): string {
    return `${playerId}:${trait}`
}
 ...
 */
export function resolveSymbiosisPropagation(
    links: readonly SymbiosisLink[],
    directLevelUps: readonly DirectLevelUp[],
): SymbiosisPropagationTarget[] {
    ...
    for (const link of links) {
        ...
        if (
            !current ||
            `${link.ownerPlayerId}:${link.sourceTrait}:${link.targetPlayerId}:${link.targetTrait}` <
                `${current.ownerPlayerId}:${current.sourceTrait}:${current.targetPlayerId}:${current.targetTrait}`
        )
    ...
    for (const [pairKey, link] of [...pairLinks.entries()].sort(([left], [right]) => left.localeCompare(right))) {
        ...
        if (levelUps.has(sourceKey))
            addRequest(link.targetPlayerId, link.targetTrait, { playerId: link.ownerPlayerId, trait: link.sourceTrait })
        if (levelUps.has(targetKey))
            addRequest(link.ownerPlayerId, link.sourceTrait, { playerId: link.targetPlayerId, trait: link.targetTrait })
    }
    return [...requests.values()]
        .map((request) => ({
            ...request,
            sourceLevelUps: [...request.sourceLevelUps].sort((left, right) =>
                endpointKey(left.playerId, left.trait).localeCompare(endpointKey(right.playerId, right.trait)),
            ),
            pairKeys: [...request.pairKeys].sort(),
        }))
        .sort((left, right) =>
            endpointKey(left.targetPlayerId, left.targetTrait).localeCompare(
                endpointKey(right.targetPlayerId, right.targetTrait),
            ),
        )
}

export function applySymbiosisLevelIncrease(
    levelBefore: AdaptationLevel,
    requestedLevels: number,
): { appliedLevels: number; levelAfter: AdaptationLevel } {
...
export function createSymbiosisPropagationEvent(
    target: SymbiosisPropagationTarget,
    levelBefore: AdaptationLevel,
): SymbiosisPropagationEvent {
