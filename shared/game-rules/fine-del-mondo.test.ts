...
const fine = (
    ...
    outcome: FineDelMondoActivation['outcome'],
): FineDelMondoActivation => ({ ownerPlayerId, activatedRound, outcome })
...
function resolve(overrides: Partial<Parameters<typeof buildPersistedRoundResolution>[0]> = {}) {
    return buildPersistedRoundResolution({
        ...
        fineDelMondoActivationOutcomes: [fine('p1', 3, 'FINE_DEL_MONDO')],
        priorRoundValues: [],
        startedAt: null,
        ...overrides,
    })
...
describe('FINE_DEL_MONDO', () => {
    it('activates from round 3 as a zero-value action without changing genes', () => {
        ...
        expect(result.resolution_data).toMatchObject({
            ...
            scheduledRoundsAfter: 5,
            fineDelMondoActivationsAfter: [fine('p1', 3, 'FINE_DEL_MONDO')],
        })
    })

    it('rejects activation before round 3, when unequipped, and after consumption', () => {
        expect(() =>
            resolve({ roundNumber: 2, fineDelMondoActivationOutcomes: [fine('p1', 2, 'FINE_DEL_MONDO')] }),
        ).toThrow('before round 3')
        expect(() => resolve({ player1CombatMutationLoadout: passiveLoadout })).toThrow('not equipped')
        expect(() => resolve({ fineDelMondoActivations: [fine('p1', 3, 'ERA_PROSPERA')] })).toThrow(
    ...
    it('applies prosperity, simultaneous outcomes, and clamps dynamically', () => {
        expect(
            resolve({ fineDelMondoActivationOutcomes: [fine('p1', 3, 'ERA_PROSPERA')] }).resolution_data
                .scheduledRoundsAfter,
        ).toBe(10)
        ...
        expect(
            resolve({
                ...
                roundNumber: 3,
                fineDelMondoActivationOutcomes: [fine('p1', 3, 'FINE_DEL_MONDO')],
            }).resolution_data.scheduledRoundsAfter,
        ).toBe(5)
        expect(
            resolve({
                scheduledRounds: 10,
                roundNumber: 3,
                fineDelMondoActivationOutcomes: [fine('p1', 3, 'ERA_PROSPERA')],
            }).resolution_data.scheduledRoundsAfter,
        ).toBe(10)
    })
    ...
    it('uses dynamic clinch and resolves tiebreaks at the scheduled deadline', () => {
        ...
        expect(
            resolveMatchOutcome({
                ...
                player1Score: 3,
                player2Score: 0,
                resolvedRoundNumber: 3,
                scheduledRounds: 5,
                storedRoundValues: [],
            }),
        ).toMatchObject({ finished: true, reason: 'CLINCH', winnerId: 'p1' })
        expect(
            resolveMatchOutcome({
                player1Id: 'p1',
                player2Id: 'p2',
                player1Score: 3,
                player2Score: 2,
                resolvedRoundNumber: 5,
                scheduledRounds: 5,
                storedRoundValues: [],
            }),
        ).toMatchObject({ finished: true, reason: 'SCORE', winnerId: 'p1' })
        expect(
            resolveMatchOutcome({
                player1Id: 'p1',
                player2Id: 'p2',
                ...
                resolvedRoundNumber: 5,
                scheduledRounds: 5,
                storedRoundValues: [{ player1Value: 1, player2Value: 0 }],
            }),
        ).toMatchObject({ finished: true, reason: 'ROUND_VALUE_TIEBREAK', winnerId: 'p1' })
    })
    ...
    it('rejects a duration change that would put the scheduled deadline in the past', () => {
        expect(() =>
            resolve({
                scheduledRounds: 7,
                roundNumber: 6,
                fineDelMondoActivationOutcomes: [fine('p1', 6, 'FINE_DEL_MONDO')],
            }),
        ).toThrow('cannot move the deadline')
    })
})
