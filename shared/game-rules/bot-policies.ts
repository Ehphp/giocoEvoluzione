...
export type BotDecisionContext = {
    roundNumber: number; scheduledRounds?: number; ownScore: number; opponentScore: number; ruleVersion: string
...
export const greedyUsePolicy: BotPolicy = { id: 'greedy-immediate-use', selectAction(context) {
    ...
    return best(candidates.map((action) => ({ action, score: action.actionType === 'USE' ? getAdaptationRoundValue(context.roundEvent, context.adaptations, action.trait, context.combatMutationState, context.combatMutationLoadout) : getEvolutionRoundValue(context.roundEvent, context.adaptations, action.trait, context.combatMutationState, context.combatMutationLoadout), reason: action.actionType === 'USE' ? 'Massimizza il valore immediato.' : 'Nessun USE disponibile.' })), context.random)
} }
...
export function evaluateHeuristicAction(context: BotDecisionContext, action: BotRoundAction, weights: HeuristicWeights = DEFAULT_HEURISTIC_WEIGHTS): HeuristicActionEvaluation {
    const scheduledRounds = context.scheduledRounds ?? STANDARD_SCHEDULED_ROUNDS; const remaining = scheduledRounds - context.roundNumber + 1; const behind = Math.max(0, context.opponentScore - context.ownScore); const decisive = Math.abs(context.ownScore - context.opponentScore) <= 1 && remaining <= 2 ? 1 : 0; const trait = context.adaptations[action.trait]
    if (action.actionType === 'USE') {
        const immediateValue = getAdaptationRoundValue(context.roundEvent, context.adaptations, action.trait, context.combatMutationState, context.combatMutationLoadout); const matchup = estimateMatchup(action.trait, context.publicOpponentAdaptations); const conservation = context.nextRoundEvent ? getAdaptationRoundValue(context.nextRoundEvent, context.adaptations, action.trait, context.combatMutationState, context.combatMutationLoadout) : immediateValue
    ...
    const nextLevel = Math.min(MAX_ADAPTATION_LEVEL, trait.level + 1) as typeof trait.level; const recovered = trait.exhausted ? 1 : 0; const nextCombatMutationState = getCombatMutationStateAfterEvolve(context.combatMutationState, context.combatMutationLoadout); const evolution = context.nextRoundEvent ? getAdaptationRoundValue(context.nextRoundEvent, { ...context.adaptations, [action.trait]: { level: nextLevel, exhausted: false } }, action.trait, nextCombatMutationState, context.combatMutationLoadout) - getAdaptationRoundValue(context.nextRoundEvent, context.adaptations, action.trait, context.combatMutationState, context.combatMutationLoadout) : nextLevel - trait.level; const immediateValue = getEvolutionRoundValue(context.roundEvent, context.adaptations, action.trait, context.combatMutationState, context.combatMutationLoadout); const remainingRounds = (MAX_ADAPTATION_LEVEL - trait.level + recovered) * remaining / scheduledRounds
    return { action, score: immediateValue + evolution * weights.evolution + recovered * weights.conservation + remainingRounds * weights.remainingRounds - behind * weights.scorePressure * 0.3 + decisive * weights.decisiveRound * 0.2, reason: `${recovered ? 'Recupero e ' : ''}investimento futuro ${evolution} con ${remaining} round residui.`, components: { immediateValue, matchup: 0, level: 0, conservation: recovered, evolution, remainingRounds, scorePressure: -behind * 0.3, decisiveRound: decisive * 0.2 } }
}
...
export function createParametricPolicy(options: ParametricPolicyOptions): BotPolicy {
    return { id: options.id, selectAction(context) {
        ...
        if (canEvolve && requested && (options.evolveThreshold === undefined || getAdaptationRoundValue(context.roundEvent, context.adaptations, greedy.trait, context.combatMutationState, context.combatMutationLoadout) <= options.evolveThreshold)) return { ...requested, reason: 'Regola parametrica di evoluzione.' }
        const uses = context.legalActions.filter((action) => action.actionType === 'USE')
        if (options.useMatchup && uses.length) return best(uses.map((action) => ({ action, score: getAdaptationRoundValue(context.roundEvent, context.adaptations, action.trait, context.combatMutationState, context.combatMutationLoadout) + estimateMatchup(action.trait, context.publicOpponentAdaptations), reason: 'Regola parametrica: valore e matchup.' })), context.random)
        return greedy
    } }
}
