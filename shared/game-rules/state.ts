...
export function createInitialAdaptations(): AdaptationCollection {
    return Object.fromEntries(
        ADAPTATION_IDS.map((adaptation) => [adaptation, { level: 0, exhausted: false }]),
    ) as AdaptationCollection
}
export function normalizeAdaptationCollection(
    ...
    for (const adaptation of ADAPTATION_IDS) {
        const state = value?.[adaptation]
        if (!state) continue
        if (typeof state.level === 'number' && Number.isFinite(state.level))
            adaptations[adaptation].level = Math.max(
                0,
                Math.min(MAX_ADAPTATION_LEVEL, Math.trunc(state.level)),
            ) as AdaptationState['level']
        if (typeof state.exhausted === 'boolean') adaptations[adaptation].exhausted = state.exhausted
    }
    return adaptations
}
export function createInitialCombatMutationState(): CombatMutationState {
    return {
...
export function isCombatMutationState(value: unknown): value is CombatMutationState {
    ...
    return (
        keys.length === 4 &&
        keys.every((key) =>
            ['elasticLimbsUsed', 'adaptiveCoreStatus', 'armoredMemoryUsed', 'recoverySurgeUsed'].includes(key),
        ) &&
        typeof value.elasticLimbsUsed === 'boolean' &&
        (value.adaptiveCoreStatus === 'DORMANT' ||
            value.adaptiveCoreStatus === 'ARMED' ||
            value.adaptiveCoreStatus === 'CONSUMED') &&
...
export function parseCombatMutationState(value: unknown, field = 'combat_mutation_state'): CombatMutationState {
    ...
    return {
        elasticLimbsUsed: value.elasticLimbsUsed,
        adaptiveCoreStatus: value.adaptiveCoreStatus,
        armoredMemoryUsed: value.armoredMemoryUsed,
        recoverySurgeUsed: value.recoverySurgeUsed,
    }
}
...
function isAdaptationId(value: unknown): value is AdaptationId {
    return typeof value === 'string' && ADAPTATION_IDS.includes(value as AdaptationId)
}
export function isSymbiosisLink(value: unknown): value is SymbiosisLink {
    ...
    return (
        keys.length === 5 &&
        keys.every((key) =>
            ['ownerPlayerId', 'sourceTrait', 'targetPlayerId', 'targetTrait', 'activatedRound'].includes(key),
        ) &&
...
export function canonicalCombatMutationLoadoutCacheKey(loadout: CombatMutationLoadout): string {
    return [...loadout]
        .sort((left, right) => COMBAT_MUTATION_IDS.indexOf(left) - COMBAT_MUTATION_IDS.indexOf(right))
        .join(',')
}

export function isSupportedRuleVersion(value: unknown): value is (typeof SUPPORTED_RULE_VERSIONS)[number] {
    return (
        typeof value === 'string' && SUPPORTED_RULE_VERSIONS.includes(value as (typeof SUPPORTED_RULE_VERSIONS)[number])
    )
}
export function assertSupportedRuleVersion(value: unknown): asserts value is (typeof SUPPORTED_RULE_VERSIONS)[number] {
    if (!isSupportedRuleVersion(value))
        throw new UnsupportedRuleVersionError(typeof value === 'string' ? value : String(value ?? 'missing'))
}
export function getAdaptationLabel(adaptation: AdaptationId): string {
    return ADAPTATION_CATALOG[adaptation].label
}
export function getRoundEventById(eventId: string): EnvironmentalCrisisDefinition {
    const roundEvent = ROUND_EVENT_BY_ID[eventId]
    if (!roundEvent) throw new Error(`Unknown environmental crisis "${eventId}".`)
    return roundEvent
}
export function getRoundEventForRound(sequence: string[], roundNumber: number): EnvironmentalCrisisDefinition | null {
