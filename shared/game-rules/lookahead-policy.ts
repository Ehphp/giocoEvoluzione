...
function decide(context: BotDecisionContext, events: readonly EnvironmentalCrisisDefinition[], depth: number, stats?: LookaheadStats): BotDecision {
    ...
    const choices = context.legalActions.map((action) => {
        ...
        for (const rival of rivals) {
            const resolution = resolveRound({ roundNumber: root.round, roundEvent: context.roundEvent, player1Id: 'own', player2Id: 'rival', player1Traits: root.own, player2Traits: root.rival, ruleVersion: context.ruleVersion, player1CombatMutationLoadout: root.ownCombatMutationLoadout, player2CombatMutationLoadout: root.rivalCombatMutationLoadout, player1CombatMutationState: root.ownCombatMutationState, player2CombatMutationState: root.rivalCombatMutationState, player1Action: { playerId: 'own', ...action }, player2Action: { playerId: 'rival', ...rival } })
