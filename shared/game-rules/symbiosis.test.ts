...
const link = (
    sourceTrait: SymbiosisLink['sourceTrait'],
    targetTrait: SymbiosisLink['targetTrait'],
    ownerPlayerId = 'p1',
    targetPlayerId = 'p2',
): SymbiosisLink => ({ ownerPlayerId, sourceTrait, targetPlayerId, targetTrait, activatedRound: 1 })
function traits(
    overrides: Partial<
        Record<keyof AdaptationCollection, Partial<AdaptationCollection[keyof AdaptationCollection]>>
    > = {},
): AdaptationCollection {
    const result = createInitialAdaptations()
    for (const [trait, state] of Object.entries(overrides))
        Object.assign(result[trait as keyof AdaptationCollection], state)
    return result
}
function round(overrides: Partial<ResolveRoundInput> = {}) {
    return resolveRound({
        ...
        player1CombatMutationLoadout: symbiosis,
        player2CombatMutationLoadout: symbiosis,
        player1Action: { playerId: 'p1', trait: 'FEROCITY', actionType: 'USE' },
        player2Action: { playerId: 'p2', trait: 'ARMOR', actionType: 'USE' },
        ...overrides,
    })
}
...
describe('SYMBIOSIS', () => {
    ...
    it('requires the equipped mutation and rejects a second activation', () => {
        expect(() =>
            round({
                player1CombatMutationLoadout: passive,
                player1Action: {
                    playerId: 'p1',
                    actionType: 'ACTIVATE_MUTATION',
                    mutationId: 'SYMBIOSIS',
        ...
        expect(() =>
            round({
                roundNumber: 2,
                symbiosisLinks: [link('FEROCITY', 'ARMOR')],
                player1Action: {
                    playerId: 'p1',
                    actionType: 'ACTIVATE_MUTATION',
                    mutationId: 'SYMBIOSIS',
    ...
    it('reflects only genuine direct EVOLVE level-ups in both directions without changing exhaustion', () => {
        const first = round({
            ...
            player2Traits: traits({ ARMOR: { exhausted: true } }),
    ...
    it('does not reflect a max-level recovery and caps a reflected increase', () => {
        ...
        const capped = round({
            roundNumber: 2,
            symbiosisLinks: [link('FEROCITY', 'ARMOR')],
            player1Traits: traits({ FEROCITY: { level: 1 } }),
            player2Traits: traits({ ARMOR: { level: 2, exhausted: true } }),
            player1Action: { playerId: 'p1', trait: 'FEROCITY', actionType: 'EVOLVE' },
            player2Action: { playerId: 'p2', trait: 'SENSES', actionType: 'USE' },
        })
    ...
    it('deduplicates inverse links for propagation while retaining both consumed slots', () => {
        ...
        const result = round({
            roundNumber: 2,
            symbiosisLinks: [link('FEROCITY', 'ARMOR'), inverse],
            player1Action: { playerId: 'p1', trait: 'FEROCITY', actionType: 'EVOLVE' },
            player2Action: { playerId: 'p2', trait: 'SENSES', actionType: 'USE' },
        })
    ...
    it('aggregates distinct overlapping links, is order independent, and never recurses', () => {
        const links: SymbiosisLink[] = [
            link('FEROCITY', 'ARMOR', 'a', 'b'),
            link('SENSES', 'ARMOR', 'c', 'b'),
            link('ARMOR', 'AGILITY', 'b', 'd'),
        ]
        const ups = [
            { playerId: 'a', trait: 'FEROCITY' as const },
            { playerId: 'c', trait: 'SENSES' as const },
        ]
