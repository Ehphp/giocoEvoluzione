...
export function assertAuditEquivalence(): void {
    ...
    for (const loadout of loadouts)
        for (const event of ROUND_EVENT_DEFINITIONS)
            for (const action of getLegalBotActions(createInitialAdaptations())) {
                const opponent = {
                    trait: ADAPTATION_IDS.find((gene) => gene !== action.trait)!,
                    actionType: 'EVOLVE' as const,
                }
                const resolution = resolveRound({
                    roundNumber: 1,
                    roundEvent: event,
                    player1Id: 'audit-left',
                    player2Id: 'audit-right',
                ...
                if (resolution.player1.breakdown.mutationBonus !== expectedBonus)
                    throw new Error('Combat Mutation loadout audit mismatch.')
            }
}
export function generateSequences(): string[][] {
    const result: string[][] = []
    const values = ROUND_EVENT_DEFINITIONS.map((event) => event.id)
    const visit = (index: number) => {
        if (index === values.length) {
            result.push([...values])
            return
        }
        for (let swap = index; swap < values.length; swap += 1) {
